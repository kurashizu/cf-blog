...
> Override per environment via the `APEX_DOMAIN` env var.
> Custom domains are still configured per-worker in the Cloudflare Dashboard.
...
| D1 `cache_entries:llm-leaderboard` | D1 | Artificial Analysis V2 API (`/api/v2/language/models/free`, paginated) | Needs `ARTIFICIAL_ANALYSIS_API_KEY` secret. Cache-worker refreshes at most every 2 h to stay under the 100 req/day Free quota. |
...
Separate Next.js Cloudflare Worker using the same `@opennextjs/cloudflare` build as `cf-blog`. URL: `AGENT_URL` from `shared/site-config.ts` (currently `https://agent.krsz.in`). No R2 — talks to Gemini directly for chat + tool calling. Default model: `gemma-4-31b-it`.
