...
export default {
	...
	'zh-CN': {
		...
		'chrome.privacy.section2Footprint': '足迹（留言板页）—— 请求本站服务器记录 Cloudflare 边缘节点已为该请求解析出的信息：国家、时区、服务节点，以及不含版本号的浏览器与操作系统类别。不存 IP、城市或精确位置；每天一枚，公开显示在足迹墙上。',
	...
	'zh-TW': {
		...
		'chrome.privacy.section2Footprint': '足跡（留言板頁）—— 請求本站伺服器記錄 Cloudflare 邊緣節點已為該請求解析出的資訊：國家、時區、服務節點，以及不含版本號的瀏覽器與作業系統類別。不存 IP、城市或精確位置；每天一枚，公開顯示在足跡牆上。',
	...
	ja: {
		...
		'chrome.privacy.section2Footprint': '足あと（ゲストブックタブ）—— Cloudflare のエッジがそのリクエストについて既に解決している情報（国、タイムゾーン、配信拠点）と、バージョン番号を除いたブラウザ／OS の種類を、このサイトのサーバーに記録させます。IP アドレス、都市、正確な位置は保存しません。1 日 1 回まで、ウォールに公開されます。',
	...
	ko: {
		...
		'chrome.privacy.section2Footprint': '발자국(방명록 탭) — Cloudflare 엣지가 해당 요청에 대해 이미 확인한 정보(국가, 시간대, 서비스 노드)와 버전 번호를 뺀 브라우저·OS 종류를 이 사이트 서버에 기록하도록 요청합니다. IP 주소, 도시, 정확한 위치는 저장하지 않습니다. 하루 한 번, 벽에 공개 표시됩니다.',
