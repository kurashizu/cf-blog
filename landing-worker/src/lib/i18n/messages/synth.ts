...
export default {
	...
	'zh-CN': {
		...
		'synth.preset.percussionOnHint': '{track} 处于打击乐模式 — 每个按键都有自己的音色；机架和音色编辑的是当前按键。点击可恢复为整条音轨一个音色（按键音色表会保留）。',
	...
	'zh-TW': {
		...
		'synth.preset.percussionOnHint': '{track} 處於打擊樂模式 — 每個按鍵都有自己的音色；機架和音色編輯的是目前按鍵。點擊可恢復為整條音軌一個音色（按鍵音色表會保留）。',
	...
	ja: {
		...
		'synth.preset.percussionOnHint': '{track} はパーカッションモードです — 各キーが個別のサウンドを持ち、ラックとパッチはアクティブなキーを編集します。クリックするとトラック1音色に戻ります（キー音色テーブルは保持されます）。',
	...
	ko: {
		...
		'synth.preset.percussionOnHint': '{track}은(는) 퍼커션 모드입니다 — 각 건반마다 고유한 사운드를 가지며, 랙과 패치는 활성 건반을 편집합니다. 클릭하면 트랙당 하나의 사운드로 되돌아갑니다(건반 음색 테이블은 유지됩니다).',
