<script lang="ts">
	...
	const row =
		'press w-full text-left px-2.5 py-1.5 flex items-center justify-between gap-3 cursor-pointer transition-colors';
</script>
...
<button
	...
	class="press flex items-center cursor-pointer transition-colors shrink-0 {open
		? 'text-white'
...
{#if open}
	...
	>
		...
		<button
			onclick={() => pick('auto')}
			class="{row} {$localeAuto
				? 'text-white bg-white/10 font-bold'
				: 'text-white/80 hover:bg-white/10'}"
		>
			<span class="flex items-center gap-2 min-w-0">
				<span class="shrink-0 {$localeAuto ? 'text-[#98c379]' : 'text-white/25'}"
					>{$localeAuto ? '●' : '○'}</span
				>
				<span class="truncate">{$t('common.lang.auto')}</span>
			</span>
			<span class="shrink-0 text-[10px] text-white/40"
				>{$t('common.lang.autoNote', { name: detected.native })}</span
			>
		</button>
		...
		{#each LOCALES as l (l.id)}
			...
			<button
				onclick={() => pick(l.id)}
				class="{row} {on ? 'text-white bg-white/10 font-bold' : 'text-white/80 hover:bg-white/10'}"
				lang={l.id}
			>
				<span class="flex items-center gap-2 min-w-0">
