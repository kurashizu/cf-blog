...
<div class="space-y-3 min-w-0">
	<div class="text-[11px] sm:text-xs text-white/60 font-mono leading-relaxed">
	...
	{#if phase === 'idle' && downloadResult === null && !error}
		<div class="text-[11px] font-mono text-white/50">{$t('utilities.speed.idle')}</div>
	{/if}
	...
	<div class="border rounded-xs bg-black/25 p-2.5 border-[#56b6c2]/20 min-w-0">
		<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
			...
			<span class="text-[10px] font-mono text-white/50">{$t('utilities.speed.section.latency.note')}</span>
		</div>
		<div class="grid grid-cols-3 gap-1.5">
			{#each [{ label: $t('utilities.speed.latency.min'), v: latencyMin }, { label: $t('utilities.speed.latency.median'), v: latencyMedian }, { label: $t('utilities.speed.latency.jitter'), v: latencyJitter }] as row (row.label)}
				<div class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0">
					<span class="text-[10px] font-mono font-bold text-white/60 uppercase truncate">{row.label}</span>
	...
	<div class="border rounded-xs bg-black/25 p-2.5 border-[#98c379]/20 min-w-0">
		<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
			...
			<span class="text-[10px] font-mono text-white/50">{$t('utilities.speed.section.download.note')}</span>
		</div>
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
			<div class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0">
				<span class="text-[10px] font-mono font-bold text-white/60 uppercase truncate">{$t('utilities.speed.download.live')}</span>
			...
			<div class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0">
				<span class="text-[10px] font-mono font-bold text-white/60 uppercase truncate">{$t('utilities.speed.download.avg')}</span>
	...
	<div class="border rounded-xs bg-black/25 p-2.5 border-[#e5c07b]/20 min-w-0">
		<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
			...
			<span class="text-[10px] font-mono text-white/50">{$t('utilities.speed.section.upload.note')}</span>
		</div>
		<div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
			<div class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0">
				<span class="text-[10px] font-mono font-bold text-white/60 uppercase truncate">{$t('utilities.speed.upload.live')}</span>
			...
			<div class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0">
				<span class="text-[10px] font-mono font-bold text-white/60 uppercase truncate">{$t('utilities.speed.upload.avg')}</span>
	...
	<div class="border border-white/15 bg-black/40 rounded-xs px-2.5 py-1.5 flex items-baseline justify-between gap-2 min-w-0">
		<span class="text-[10px] font-mono font-bold text-white/60 uppercase shrink-0">{$t('utilities.speed.totalMoved')}</span>
