<script lang="ts">
	...
	async function enable() {
		playSound('click');
		const DOE = window.DeviceOrientationEvent as unknown as Partial<PermissionCapable> &
			typeof DeviceOrientationEvent;
		const DME = window.DeviceMotionEvent as unknown as Partial<PermissionCapable> &
	...
	function startAmbientLight() {
		const Ctor = (
			window as unknown as {
				AmbientLightSensor?: new (opts?: {
					frequency?: number;
				}) => EventTarget & { illuminance?: number; start: () => void; stop: () => void };
			}
		).AmbientLightSensor;
		if (!Ctor) {
		...
		try {
			...
			lightState =
				e instanceof Error && (e.name === 'SecurityError' || e.name === 'NotAllowedError')
					? 'blocked'
					: 'error';
	...
	onMount(() => {
		const DOE = window.DeviceOrientationEvent as unknown as
			Partial<{ requestPermission: unknown }> | undefined;
		const DME = window.DeviceMotionEvent as unknown as
			Partial<{ requestPermission: unknown }> | undefined;
		const needsPermission =
			typeof DOE?.requestPermission === 'function' || typeof DME?.requestPermission === 'function';
		if (needsPermission) {
			needsGesture = true;
		} else if (
	...
	function deg(v: number | null): string {
		return v === null
			? tr('utilities.sensors.orientation.na')
			: tr('utilities.sensors.orientation.deg', { deg: Math.round(v * 10) / 10 });
	}
	...
	let heading = $derived.by(() => {
		...
		if (typeof orientation.webkitCompassHeading === 'number') {
			return {
				deg: Math.round(orientation.webkitCompassHeading),
				source: tr('utilities.sensors.compass.source.webkit')
			};
		}
		if (orientation.absolute && orientation.alpha !== null) {
			return {
				deg: Math.round((360 - orientation.alpha) % 360),
				source: tr('utilities.sensors.compass.source.alpha')
			};
		}
		return null;
	});
...
<div class="space-y-3 min-w-0">
	{#if needsGesture}
		<div
			class="border border-white/15 bg-black/40 rounded-xs p-3 flex flex-wrap items-center gap-2"
		>
			<button
				onclick={enable}
	...
	{#if gotAnyEvent}
		<div class="flex flex-wrap items-center gap-2">
			<span
				class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60 text-xs font-mono"
			>
				{$t('utilities.sensors.eventRate')}
				<span class="font-bold text-[#98c379]"
					>{eventRateHz === null
						? $t('utilities.sensors.eventRate.na')
						: $t('utilities.sensors.eventRate.hz', { hz: eventRateHz })}</span
				>
			</span>
		</div>
		...
		<div class="border rounded-xs bg-black/25 p-2.5 border-[#56b6c2]/20 min-w-0">
			<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
				<span class="text-xs font-black font-mono" style="color: #56b6c2"
					>{$t('utilities.sensors.section.orientation')}</span
				>
			</div>
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-1.5">
				...
				>
					<span class="text-[10px] font-mono font-bold text-white/45 truncate"
						>{$t('utilities.sensors.orientation.alpha')}</span
					>
					<span class="text-xs font-mono font-bold text-[#56b6c2] truncate"
						>{deg(orientation?.alpha ?? null)}</span
					>
				</div>
				<div
					class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0"
				>
					<span class="text-[10px] font-mono font-bold text-white/45 truncate"
						>{$t('utilities.sensors.orientation.beta')}</span
					>
					<span class="text-xs font-mono font-bold text-[#56b6c2] truncate"
						>{deg(orientation?.beta ?? null)}</span
					>
				</div>
				<div
					class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0"
				>
					<span class="text-[10px] font-mono font-bold text-white/45 truncate"
						>{$t('utilities.sensors.orientation.gamma')}</span
					>
					<span class="text-xs font-mono font-bold text-[#56b6c2] truncate"
						>{deg(orientation?.gamma ?? null)}</span
					>
				</div>
				<div
					class="border border-white/10 bg-black/40 rounded-xs px-2 py-1.5 flex flex-col gap-0.5 min-w-0"
				>
					<span class="text-[10px] font-mono font-bold text-white/45 truncate"
						>{$t('utilities.sensors.orientation.absolute')}</span
					>
					<span class="text-xs font-mono font-bold text-[#d8dee9] truncate"
						>{orientation?.absolute
							? $t('utilities.sensors.orientation.yes')
							: $t('utilities.sensors.orientation.no')}</span
					>
				</div>
			</div>
		</div>
			...
			<div class="border rounded-xs bg-black/25 p-2.5 border-[#e5c07b]/20 min-w-0">
				<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
					<span class="text-xs font-black font-mono" style="color: #e5c07b"
						>{$t('utilities.sensors.section.motion')}</span
					>
				</div>
				<div class="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
					...
					>
						<span class="text-[10px] font-mono font-bold text-white/45 uppercase truncate"
							>{$t('utilities.sensors.motion.acceleration')} ({$t(
								'utilities.sensors.motion.ms2'
							)})</span
						>
						<span class="text-xs font-mono font-bold text-[#e5c07b] truncate"
							>{$t('utilities.sensors.motion.axes', {
								x: ms2(motion.acceleration.x),
								y: ms2(motion.acceleration.y),
								z: ms2(motion.acceleration.z)
							})}</span
						>
					</div>
					<div
					...
					>
						<span class="text-[10px] font-mono font-bold text-white/45 uppercase truncate"
							>{$t('utilities.sensors.motion.accelerationGravity')} ({$t(
								'utilities.sensors.motion.ms2'
							)})</span
						>
						<span class="text-xs font-mono font-bold text-[#e5c07b] truncate"
							>{$t('utilities.sensors.motion.axes', {
								x: ms2(motion.accelerationIncludingGravity.x),
								y: ms2(motion.accelerationIncludingGravity.y),
								z: ms2(motion.accelerationIncludingGravity.z)
							})}</span
						>
					</div>
					<div
					...
					>
						<span class="text-[10px] font-mono font-bold text-white/45 uppercase truncate"
							>{$t('utilities.sensors.motion.rotationRate')} ({$t(
								'utilities.sensors.motion.degs'
							)})</span
						>
						<span class="text-xs font-mono font-bold text-[#e5c07b] truncate"
							>{$t('utilities.sensors.motion.axesDeg', {
								alpha: degs(motion.rotationRate.alpha),
								beta: degs(motion.rotationRate.beta),
								gamma: degs(motion.rotationRate.gamma)
							})}</span
						>
					</div>
					<div
					...
					>
						<span class="text-[10px] font-mono font-bold text-white/45 uppercase truncate"
							>{$t('utilities.sensors.motion.interval')}</span
						>
						<span class="text-xs font-mono font-bold text-[#d8dee9] truncate"
							>{motion.interval === null
								? $t('utilities.sensors.motion.na')
								: $t('utilities.sensors.motion.interval.ms', {
										ms: Math.round(motion.interval * 100) / 100
									})}</span
						>
					</div>
				</div>
			...
			<div class="border rounded-xs bg-black/25 p-2.5 border-[#61afef]/20 min-w-0">
				<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
					<span class="text-xs font-black font-mono" style="color: #61afef"
						>{$t('utilities.sensors.section.level')}</span
					>
				</div>
				<canvas bind:this={canvasEl} width="180" height="180" class="mx-auto block max-w-full"
				></canvas>
				<div class="text-[10px] font-mono text-white/35 mt-1.5 text-center">
					{$t('utilities.sensors.level.hint')}
				</div>
			</div>
			...
			<div class="border rounded-xs bg-black/25 p-2.5 border-[#c678dd]/20 min-w-0 flex flex-col">
				<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
					<span class="text-xs font-black font-mono" style="color: #c678dd"
						>{$t('utilities.sensors.section.compass')}</span
					>
				</div>
				{#if heading}
					<div class="flex-1 flex flex-col items-center justify-center gap-1.5">
						<div class="text-2xl font-mono font-black text-[#c678dd]">
							{$t('utilities.sensors.compass.deg', { deg: heading.deg })}
						</div>
						<div
							class="w-full h-2 bg-black/60 border border-white/10 rounded-xs relative overflow-hidden"
						>
							<div
								class="absolute top-0 bottom-0 w-1.5 rounded-xs bg-[#c678dd]"
								style="left: calc({(heading.deg / 360) * 100}% - 3px)"
							></div>
						</div>
				...
				{:else}
					<div
						class="flex-1 flex items-center justify-center text-[11px] font-mono text-white/35 text-center"
					>
						{$t('utilities.sensors.compass.na')}
					</div>
				{/if}
			</div>
		</div>
	...
	<div class="border rounded-xs bg-black/25 p-2.5 border-[#98c379]/20 min-w-0">
		<div class="flex items-baseline justify-between gap-2 border-b border-white/10 pb-1 mb-1.5">
			<span class="text-xs font-black font-mono" style="color: #98c379"
				>{$t('utilities.sensors.section.light')}</span
			>
		</div>
		...
		{:else if lightState === 'error'}
			<div class="text-xs font-mono text-[#e06c75]">
				{$t('utilities.sensors.light.error', { message: lightErrorMessage })}
			</div>
		{:else}
			<div
				class="border border-white/10 bg-black/40 rounded-xs px-2.5 py-1.5 flex items-baseline justify-between gap-2 min-w-0 max-w-xs"
			>
				<span class="text-[10px] font-mono font-bold text-white/45 uppercase shrink-0"
					>{$t('utilities.sensors.light.value')}</span
				>
				<span class="text-xs font-mono font-bold text-[#98c379] truncate"
					>{lux === null
						? $t('utilities.sensors.eventRate.na')
						: $t('utilities.sensors.light.lux', { lux })}</span
				>
			</div>
		{/if}
	</div>
