<script lang="ts">
	...
	let avg = $derived(
		results.length ? Math.round(results.reduce((a, b) => a + b, 0) / results.length) : null
	);
...
<div class="space-y-2">
	<div class="flex flex-wrap items-center gap-1.5 text-xs font-mono">
		<span class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60">
			{$t('utilities.reaction.last')}
			<span class="font-black text-[#e5c07b] inline-block"
				>{#key `${lastMs}-${resultGen}`}<span class="value-in inline-block"
						>{lastMs === null ? '—' : $t('utilities.reaction.ms', { ms: lastMs })}</span
					>{/key}</span
			>
		</span>
		<span class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60">
			{$t('utilities.reaction.best')}
			<span class="font-black text-[#98c379] inline-block"
				>{#key best}<span class="value-in inline-block"
						>{best === null ? '—' : $t('utilities.reaction.ms', { ms: best })}</span
					>{/key}</span
			>
		</span>
		<span class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60">
			{$t('utilities.reaction.avg')}
			<span class="font-black text-[#56b6c2] inline-block"
				>{#key avg}<span class="value-in inline-block"
						>{avg === null ? '—' : $t('utilities.reaction.ms', { ms: avg })}</span
					>{/key}</span
			>
	...
	<button
		...
		class="press w-full min-h-[260px] rounded-xs border font-mono cursor-pointer transition-colors duration-100 flex flex-col items-center justify-center gap-2 select-none
			{phase === 'waiting'
			? 'bg-[#e06c75]/25 border-[#e06c75]'
			: phase === 'go'
				? 'bg-[#98c379]/30 border-[#98c379]'
				: 'bg-black/50 border-white/15 hover:border-white/40'}
	...
	>
		...
		{:else}
			<span class="text-3xl font-black text-[#e5c07b] value-in"
				>{$t('utilities.reaction.ms', { ms: lastMs ?? 0 })}</span
			>
