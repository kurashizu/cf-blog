<script lang="ts">
	...
	const nav =
		typeof navigator !== 'undefined'
			? (navigator as Navigator & { usb?: UsbApi; hid?: HidApi; serial?: SerialApi })
			: undefined;
	...
	const USAGE_PAGE_NAMES: Record<number, string> = {
	...
	const GENERIC_DESKTOP_USAGE: Record<number, string> = {
		0x01: 'pointer',
		0x02: 'mouse',
		0x04: 'joystick',
		0x05: 'gamepad',
	...
	function usagePageName(page: number | undefined): string {
		...
		return known
			? `0x${page.toString(16).padStart(2, '0')} (${known})`
			: `0x${page.toString(16).padStart(2, '0')}`;
	}
	function usageName(page: number | undefined, usage: number | undefined): string {
		...
		return known
			? `0x${usage.toString(16).padStart(2, '0')} (${known})`
			: `0x${usage.toString(16).padStart(2, '0')}`;
	}
...
<div class="space-y-2">
	...
	<div class="border border-white/15 bg-black/40 rounded-xs p-2.5 space-y-1.5 min-w-0">
		<div class="flex items-center justify-between gap-2">
			<div class="text-[10px] font-mono font-bold text-white/45 uppercase">
				{$t('utilities.usb.section.usb')}
			</div>
			{#if usbApi}
				<button
					onclick={pickUsb}
					class="press px-2 py-1 border border-[#56b6c2]/50 text-[#56b6c2] hover:bg-[#56b6c2]/20 rounded-xs font-bold text-[10px] cursor-pointer transition-colors"
				>
		...
		{:else}
			<div class="space-y-1.5">
				{#each usbDevices as d, i (i)}
					<div
						class="border border-white/10 bg-black/30 rounded-xs p-1.5 text-[10px] font-mono space-y-0.5 min-w-0"
					>
						...
						<div class="flex flex-wrap gap-x-3 gap-y-0.5 text-white/40">
							<span
								>{$t('utilities.usb.serial')}: {d.serialNumber
									? $t('utilities.usb.serial.yes')
									: $t('utilities.usb.serial.no')}</span
							>
							<span
								>{$t('utilities.usb.usbVersion')}: {d.deviceVersionMajor}.{d.deviceVersionMinor}</span
							>
							<span
								>{$t('utilities.usb.deviceClass')}: 0x{d.deviceClass
									.toString(16)
									.padStart(2, '0')}/0x{d.deviceSubclass
									.toString(16)
									.padStart(2, '0')}/0x{d.deviceProtocol.toString(16).padStart(2, '0')}</span
							>
						...
						{#each d.configurations as cfg, ci (ci)}
							<div class="text-white/35 pl-2">
								...
								{cfg.interfaces
									.map(
										(iface) =>
											`0x${(iface.alternates[0]?.interfaceClass ?? 0).toString(16).padStart(2, '0')}`
									)
									.join(', ')}
							</div>
						{/each}
						...
						>
							<div class="flex flex-wrap items-center gap-x-2 gap-y-0.5">
								<span class="font-bold text-[#98c379]"
									>{hexId(d.vendorId)}:{hexId(d.productId)}</span
								>
								<span class="text-white/70 truncate">{d.productName || 'n/a'}</span>
								<button
									onclick={() => toggleMonitor(d)}
									class="press ml-auto px-1.5 py-0.5 border rounded-xs font-bold text-[9px] cursor-pointer transition-colors {monitoringId ===
									deviceKey(d)
										? 'border-[#e06c75]/60 text-[#e06c75]'
										: 'border-white/20 text-white/50 hover:border-white/40'}"
								>
									{monitoringId === deviceKey(d)
										? $t('utilities.usb.hid.stopMonitor')
										: $t('utilities.usb.hid.monitor')}
								</button>
							</div>
							<div class="text-white/40">
								{#each d.collections as c, ci (ci)}
									<div>
										{$t('utilities.usb.hid.collection', {
											usagePage: usagePageName(c.usagePage),
											usage: usageName(c.usagePage, c.usage),
											inputs: c.inputReports?.length ?? 0,
											outputs: c.outputReports?.length ?? 0,
											features: c.featureReports?.length ?? 0
										})}
									</div>
								{/each}
							</div>
							{#if monitoringId === deviceKey(d)}
								<div
									class="border-t border-white/10 pt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-white/60"
								>
									<span>{$t('utilities.usb.hid.reportId')}: {monitorReportId ?? 'n/a'}</span>
									<span class="truncate"
										>{$t('utilities.usb.hid.bytes')}: {monitorHex || 'n/a'}</span
									>
									<span class="ml-auto shrink-0"
										>{$t('utilities.usb.hid.rate', { rate: monitorRate })}</span
									>
								</div>
							{/if}
						</div>
	...
	<div class="border border-white/15 bg-black/40 rounded-xs p-2.5 space-y-1.5 min-w-0">
		<div class="flex items-center justify-between gap-2">
			<div class="text-[10px] font-mono font-bold text-white/45 uppercase">
				{$t('utilities.usb.section.serial')}
			</div>
			{#if serialApi}
				<button
					onclick={pickSerial}
					class="press px-2 py-1 border border-[#e5c07b]/50 text-[#e5c07b] hover:bg-[#e5c07b]/20 rounded-xs font-bold text-[10px] cursor-pointer transition-colors"
				>
					{$t('utilities.usb.pickDevice')}
				</button>
			{/if}
		...
		{:else}
			<div class="space-y-1">
				{#each serialPorts as p, i (i)}
					...
					<div
						class="border border-white/10 bg-black/30 rounded-xs p-1.5 text-[10px] font-mono min-w-0"
					>
						<span class="font-bold text-[#e5c07b]"
							>{hexId(info.usbVendorId)}:{hexId(info.usbProductId)}</span
						>
					</div>
				{/each}
			</div>
