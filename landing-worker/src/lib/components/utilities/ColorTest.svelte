<script lang="ts">
	...
	function readCaps(): Caps {
		...
		return {
			colorGamut,
			dynamicRange: mm('(dynamic-range: high)')
				? 'high'
				: mm('(dynamic-range: standard)')
					? 'standard'
					: 'n/a',
			videoDynamicRange: mm('(video-dynamic-range: high)')
				? 'high'
				: mm('(video-dynamic-range: standard)')
					? 'standard'
					: 'n/a',
	...
	const WIDE_PAIRS = [
		{
			nameKey: 'utilities.color.swatch.red',
			srgb: '#ff0000',
			p3: 'color(display-p3 1 0 0)',
			rec2020: 'color(rec2020 1 0 0)'
		},
		{
			nameKey: 'utilities.color.swatch.green',
			srgb: '#00ff00',
			p3: 'color(display-p3 0 1 0)',
			rec2020: 'color(rec2020 0 1 0)'
		},
		{
			nameKey: 'utilities.color.swatch.blue',
			srgb: '#0000ff',
			p3: 'color(display-p3 0 0 1)',
			rec2020: 'color(rec2020 0 0 1)'
		}
	];
...
<div class="space-y-2">
	...
	<div class="border border-white/15 bg-black/40 rounded-xs p-2.5 space-y-1.5">
		...
		>
			{$t('utilities.color.capabilities')}
		</div>
		<div class="grid grid-cols-2 sm:grid-cols-3 gap-1.5 text-[10px] font-mono">
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.gamut')}</span>
				<span class="font-bold text-[#56b6c2]">{caps.colorGamut}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.dynamicRange')}</span>
				<span class="font-bold text-[#98c379]">{caps.dynamicRange}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.videoDynamicRange')}</span>
				<span class="font-bold text-[#98c379]">{caps.videoDynamicRange}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.colorDepth')}</span>
				<span class="font-bold text-[#e5c07b]">{caps.colorDepth}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.forcedColors')}</span>
				<span class="font-bold text-white/70">{caps.forcedColors}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.prefersContrast')}</span>
				<span class="font-bold text-white/70">{caps.prefersContrast}</span>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.cssP3')}</span>
				<span class="font-bold {caps.supportsP3 ? 'text-[#98c379]' : 'text-[#e06c75]'}"
					>{caps.supportsP3 ? $t('utilities.color.yes') : $t('utilities.color.no')}</span
				>
			</div>
			<div class="border border-white/10 bg-black/30 rounded-xs p-1.5">
				<span class="text-white/40">{$t('utilities.color.cap.cssRec2020')}</span>
				<span class="font-bold {caps.supportsRec2020 ? 'text-[#98c379]' : 'text-[#e06c75]'}"
					>{caps.supportsRec2020 ? $t('utilities.color.yes') : $t('utilities.color.no')}</span
				>
			</div>
		</div>
	</div>
	...
	<div class="grid grid-cols-2 sm:grid-cols-3 gap-1.5">
		{#each SECTIONS as s (s.id)}
			<button
				...
				class="press text-left border bg-black/40 hover:bg-white/5 rounded-xs px-2.5 py-2 cursor-pointer transition-colors"
			>
		...
		<div class="min-h-full flex flex-col items-center justify-center gap-6 p-6">
			{#if section === 'wide'}
				<div class="w-full max-w-3xl space-y-3">
					<h2 class="text-white text-sm font-mono font-bold text-center">
						{$t('utilities.color.section.wide')}
					</h2>
					{#each WIDE_PAIRS as pair (pair.nameKey)}
						<div class="space-y-1">
							...
							<div class="flex h-24 rounded-xs overflow-hidden border border-white/10">
								<div
									...
									style="background-color: {pair.srgb}"
								>
									<span class="text-[10px] font-mono text-black/50">sRGB</span>
								</div>
								<div
									class="flex-1 flex items-end justify-center pb-1"
									style="background-color: {pair.p3}"
								>
									<span class="text-[10px] font-mono text-black/50">display-p3</span>
								</div>
								<div
									class="flex-1 flex items-end justify-center pb-1"
									style="background-color: {pair.rec2020}"
								>
									<span class="text-[10px] font-mono text-black/50">rec2020</span>
								</div>
							</div>
						</div>
					{/each}
					<p class="text-white/50 text-xs font-mono text-center max-w-xl mx-auto">
						{$t('utilities.color.wideHint')}
					</p>
				</div>
			{:else if section === 'gamma'}
				<div class="w-full max-w-3xl space-y-3">
					<h2 class="text-white text-sm font-mono font-bold text-center">
						{$t('utilities.color.section.gamma')}
					</h2>
					{#each GAMMAS as g (g)}
						...
						<div class="flex items-center gap-3">
							...
							<div
								...
								style="background-color: rgb({grey},{grey},{grey})"
							></div>
							<div
								class="flex-1 h-14 rounded-xs"
								style="image-rendering: pixelated; background-image: repeating-linear-gradient(90deg, #000 0 {patternSize}px, #fff {patternSize}px {patternSize *
									2}px);"
							></div>
						</div>
					{/each}
					<p class="text-white/50 text-xs font-mono text-center max-w-xl mx-auto">
						{$t('utilities.color.gammaHint')}
					</p>
					<p class="text-white/30 text-[10px] font-mono text-center">
						{$t('utilities.color.gammaDpr', { dpr })}
					</p>
				</div>
			{:else if section === 'black'}
				<div class="w-full flex flex-col items-center gap-4" style="background: #000;">
					<h2 class="text-white/40 text-sm font-mono font-bold">
						{$t('utilities.color.section.black')}
					</h2>
					...
					<p class="text-white/30 text-xs font-mono text-center max-w-xl px-6">
						{$t('utilities.color.blackHint')}
					</p>
				</div>
			{:else if section === 'white'}
				<div class="w-full flex flex-col items-center gap-4" style="background: #fff;">
					<h2 class="text-black/50 text-sm font-mono font-bold">
						{$t('utilities.color.section.white')}
					</h2>
					<div class="flex flex-wrap items-center justify-center gap-3 px-6">
						{#each WHITE_STEPS as v (v)}
							<div class="flex flex-col items-center gap-1">
								<div
									class="w-14 h-14 rounded-xs border border-black/10"
					...
					<p class="text-black/40 text-xs font-mono text-center max-w-xl px-6">
						{$t('utilities.color.whiteHint')}
					</p>
				</div>
			{:else if section === 'saturation'}
				<div class="w-full max-w-3xl space-y-2">
					<h2 class="text-white text-sm font-mono font-bold text-center">
						{$t('utilities.color.section.saturation')}
					</h2>
					...
					{/each}
					<p class="text-white/40 text-xs font-mono text-center pt-1">
						{$t('utilities.color.saturationHint')}
					</p>
				</div>
			{:else if section === 'whitepoint'}
				<div class="w-full max-w-2xl space-y-3">
					<h2 class="text-white text-sm font-mono font-bold text-center">
						{$t('utilities.color.section.whitepoint')}
					</h2>
					<div class="flex h-24 rounded-xs overflow-hidden border border-white/10">
						{#each WHITE_POINTS as wp (wp.key)}
							<div
								class="flex-1 flex items-end justify-center pb-1"
								style="background-color: {wp.css}"
							>
								<span class="text-[10px] font-mono text-black/50">{$t(wp.key)}</span>
							</div>
						{/each}
					</div>
					<p class="text-white/50 text-xs font-mono text-center">
						{$t('utilities.color.whitePointHint')}
					</p>
				</div>
			{/if}
		</div>
