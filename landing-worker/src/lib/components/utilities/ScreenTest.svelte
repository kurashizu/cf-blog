<script lang="ts">
	...
	const PIXEL_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.white',
			kind: 'fill',
			bg: '#ffffff',
			hintDark: true
		},
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.black',
			kind: 'fill',
			bg: '#000000'
		},
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.red',
			kind: 'fill',
			bg: '#ff0000'
		},
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.green',
			kind: 'fill',
			bg: '#00ff00',
			hintDark: true
		},
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.blue',
			kind: 'fill',
			bg: '#0000ff'
		},
		{
			group: 'utilities.screen.mode.pixels.label',
			name: 'utilities.screen.step.gray50',
			kind: 'fill',
			bg: '#808080'
		}
	];
	...
	const GRAYSCALE_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.grayscale.label',
			name: 'utilities.screen.step.steps8',
			kind: 'bars',
			n: 8
		},
		{
			group: 'utilities.screen.mode.grayscale.label',
			name: 'utilities.screen.step.steps16',
			kind: 'bars',
			n: 16
		},
		{
			group: 'utilities.screen.mode.grayscale.label',
			name: 'utilities.screen.step.steps32',
			kind: 'bars',
			n: 32
		},
		{
			group: 'utilities.screen.mode.grayscale.label',
			name: 'utilities.screen.step.steps64',
			kind: 'bars',
			n: 64
		},
		{
			group: 'utilities.screen.mode.grayscale.label',
			name: 'utilities.screen.step.continuous',
			kind: 'gradient',
	...
	const GRADIENT_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.gradients.label',
			name: 'utilities.screen.step.redRamp',
			kind: 'gradient',
			css: 'linear-gradient(90deg,#000,#f00)'
		},
		{
			group: 'utilities.screen.mode.gradients.label',
			name: 'utilities.screen.step.greenRamp',
			kind: 'gradient',
			css: 'linear-gradient(90deg,#000,#0f0)'
		},
		{
			group: 'utilities.screen.mode.gradients.label',
			name: 'utilities.screen.step.blueRamp',
			kind: 'gradient',
			css: 'linear-gradient(90deg,#000,#00f)'
		},
		{
			group: 'utilities.screen.mode.gradients.label',
			name: 'utilities.screen.step.hueSweep',
			kind: 'gradient',
			css: 'linear-gradient(90deg,#f00,#ff0,#0f0,#0ff,#00f,#f0f,#f00)'
		}
	];

	const CRUSH_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.levels.label',
			name: 'utilities.screen.step.blackCrush',
			kind: 'crush',
			side: 'black'
		},
		{
			group: 'utilities.screen.mode.levels.label',
			name: 'utilities.screen.step.whiteSaturation',
			kind: 'crush',
			side: 'white'
		}
	];
	...
	const TEXT_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.text.label',
			name: 'utilities.screen.step.lightOnDark',
			kind: 'text',
			inverted: false
		},
		{
			group: 'utilities.screen.mode.text.label',
			name: 'utilities.screen.step.darkOnLight',
			kind: 'text',
			inverted: true
		}
	];

	const GHOSTING_STEPS: Step[] = [
		{
			group: 'utilities.screen.mode.ghosting.label',
			name: 'utilities.screen.step.movingBlocks',
			kind: 'ghosting'
		}
	];
	...
	const MODE_DEFS: Mode[] = [
		{
			id: 'pixels',
			labelKey: 'utilities.screen.mode.pixels.label',
			color: '#e5c07b',
			descKey: 'utilities.screen.mode.pixels.desc',
			steps: PIXEL_STEPS
		},
		{
			id: 'grayscale',
			labelKey: 'utilities.screen.mode.grayscale.label',
			color: '#98c379',
			descKey: 'utilities.screen.mode.grayscale.desc',
			steps: GRAYSCALE_STEPS
		},
		{
			id: 'gradients',
			labelKey: 'utilities.screen.mode.gradients.label',
			color: '#56b6c2',
			descKey: 'utilities.screen.mode.gradients.desc',
			steps: GRADIENT_STEPS
		},
		{
			id: 'levels',
			labelKey: 'utilities.screen.mode.levels.label',
			color: '#c678dd',
			descKey: 'utilities.screen.mode.levels.desc',
			steps: CRUSH_STEPS
		},
		{
			id: 'sharpness',
			labelKey: 'utilities.screen.mode.sharpness.label',
			color: '#e06c75',
			descKey: 'utilities.screen.mode.sharpness.desc',
			steps: SHARPNESS_STEPS
		},
		{
			id: 'text',
			labelKey: 'utilities.screen.mode.text.label',
			color: '#61afef',
			descKey: 'utilities.screen.mode.text.desc',
			steps: TEXT_STEPS
		},
		{
			id: 'ghosting',
			labelKey: 'utilities.screen.mode.ghosting.label',
			color: '#d19a66',
			descKey: 'utilities.screen.mode.ghosting.desc',
			steps: GHOSTING_STEPS
		}
	];

	let MODES = $derived(
		MODE_DEFS.map((m) => ({ ...m, label: $t(m.labelKey), desc: $t(m.descKey) }))
	);
	...
	const TEXT_SAMPLE =
		'The quick brown fox jumps over the lazy dog 0123456789 il1I|oO0 永體體験あア한';
				...
				>
					<div class="flex items-center justify-between">
						...
						<span class="text-[10px] font-mono text-white/35"
							>{mode.steps.length > 1
								? $t('utilities.screen.pattern.count.plural', { count: mode.steps.length })
								: $t('utilities.screen.pattern.count', { count: mode.steps.length })}</span
						>
					</div>
	...
	>
				...
				<div class="flex flex-wrap items-center justify-center gap-3 px-6">
					{#each step.side === 'black' ? BLACK_PATCHES : WHITE_PATCHES as lum (lum)}
						<div class="flex flex-col items-center gap-1.5">
							<div
								class="w-16 h-16 sm:w-20 sm:h-20 rounded-xs"
								style="background: hsl(0 0% {lum}%)"
							></div>
							<span
								class="text-[10px] font-mono"
								style="color: {step.side === 'black' ? '#555' : '#aaa'}">{lum}%</span
							>
						</div>
					{/each}
				</div>
				<p
					class="text-xs font-mono px-6 text-center max-w-xl"
					style="color: {step.side === 'black' ? '#666' : '#999'}"
				>
					{step.side === 'black'
						? $t('utilities.screen.crush.black.hint')
						: $t('utilities.screen.crush.white.hint')}
				</p>
			</div>
		...
		{:else if step.kind === 'text'}
			<div
				...
				style="background: {step.inverted ? '#fff' : '#000'}; color: {step.inverted
					? '#000'
					: '#fff'}"
			>
				{#each TEXT_SIZES as px (px)}
					<div style="font-size: {px}px" class="leading-snug space-y-0.5">
						<div style="font-family: 'Jelly Pixel', 'JetBrains Mono', monospace">
							{$t('utilities.screen.text.mono', { px, sample: TEXT_SAMPLE })}
						</div>
						<div style="font-family: ui-sans-serif, system-ui, sans-serif">
							{$t('utilities.screen.text.sans', { px, sample: TEXT_SAMPLE.slice(-14) })}
						</div>
						<div style="font-family: Georgia, 'Times New Roman', serif">
							{$t('utilities.screen.text.serif', { px, sample: TEXT_SAMPLE.slice(-14) })}
						</div>
					</div>
				{/each}
				<p
					class="text-xs pt-2"
					style="color: {step.inverted
						? '#888'
						: '#777'}; font-family: 'Jelly Pixel', 'JetBrains Mono', monospace"
				>
			...
			<div class="absolute inset-0 flex flex-col justify-center gap-8" style="background: #7f7f7f">
				{#each [{ key: 'utilities.screen.ghosting.slow', dur: 4 }, { key: 'utilities.screen.ghosting.medium', dur: 2 }, { key: 'utilities.screen.ghosting.fast', dur: 1 }] as lane (lane.key)}
					<div class="relative h-24">
						<span class="absolute left-3 top-1 text-[10px] font-mono font-bold text-black/50"
							>{$t(lane.key)}</span
						>
						<div
							class="ghost-box bg-white border border-black/30"
							style="animation-duration: {lane.dur}s; top: 24px;"
						></div>
						<div
							class="ghost-box bg-black border border-white/30"
							style="animation-duration: {lane.dur}s; animation-delay: -{lane.dur / 2}s; top: 24px;"
						></div>
					</div>
				{/each}
		...
		{#if hintVisible}
			...
			>
				{$t('utilities.screen.overlay.stepCounter', {
					group: $t(step.group),
					name: $t(step.name),
					index: stepIdx + 1,
					total: steps.length
