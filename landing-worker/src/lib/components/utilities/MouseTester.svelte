...
<div class="space-y-2">
	<div class="flex flex-wrap items-center gap-1.5 text-xs font-mono">
		<span class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60">
			{$t('utilities.mouse.readout.pos')}
			<span class="font-bold text-[#e5c07b]">{pos.x}, {pos.y}</span>
		</span>
		<span
			class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60"
			title={$t('utilities.mouse.readout.moveEvents.title')}
		>
			{$t('utilities.mouse.readout.moveEvents')}
			<span class="font-bold text-[#56b6c2]">{moveRate}</span>
		</span>
		<span
			class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60"
			title={$t('utilities.mouse.readout.clickGap.title')}
		>
			{$t('utilities.mouse.readout.clickGap')}
			<span class="font-bold text-[#c678dd]"
				>{lastClickGap === null
					? '—'
					: $t('utilities.mouse.readout.clickGap.value', { ms: lastClickGap })}</span
			>
		</span>
		<span class="px-2 py-1 border border-white/15 bg-black/40 rounded-xs text-white/60">
			{$t('utilities.mouse.readout.dblClicks')}
			<span class="font-bold text-[#98c379]">{dblClicks}</span>
		</span>
		<button
			onclick={reset}
	...
	<div class="grid grid-cols-1 md:grid-cols-[220px_minmax(0,1fr)] gap-2">
		...
		<div class="border border-white/15 bg-black/40 rounded-xs p-3 flex flex-col items-center gap-2">
			<div class="flex gap-1 w-full">
				<div class="flex flex-col gap-1 justify-center">
					<div
						class="border rounded-xs w-7 h-8 flex items-center justify-center text-[9px] font-mono font-bold transition-colors duration-75 {btnClass(
							4
						)}"
						style={isDown(4)
							? `background-color: ${themeStyles.cursorColor}; border-color: ${themeStyles.cursorColor};`
							: ''}
						title={$t('utilities.mouse.button.forward')}
					>
						X2
					</div>
					<div
						...
						style={isDown(3)
							? `background-color: ${themeStyles.cursorColor}; border-color: ${themeStyles.cursorColor};`
							: ''}
						title={$t('utilities.mouse.button.back')}
					>
						X1
					</div>
				</div>
				<div class="flex-1 grid grid-cols-[1fr_22px_1fr] gap-1">
					<div
						class="border rounded-tl-2xl rounded-bl-xs h-[76px] flex items-end justify-center pb-1 text-[9px] font-mono font-bold transition-colors duration-75 {btnClass(
							0
						)}"
						style={isDown(0)
							? `background-color: ${themeStyles.cursorColor}; border-color: ${themeStyles.cursorColor};`
					...
					<div class="flex flex-col items-center justify-start pt-1 gap-1">
						<div
							class="border rounded-full w-4 h-9 flex items-center justify-center text-[9px] font-mono font-bold transition-colors duration-75 {btnClass(
								1
							)}"
							style={isDown(1)
								? `background-color: ${themeStyles.cursorColor}; border-color: ${themeStyles.cursorColor};`
								: ''}
							title={$t('utilities.mouse.button.middle')}
						>
							M
						</div>
						<div
							class="text-[9px] font-mono transition-colors {lastWheelDelta < 0
								? 'text-[#56b6c2] font-bold'
								: 'text-white/30'}"
						>
							▲{wheelUp}
						</div>
					...
					<div
						class="border rounded-tr-2xl rounded-br-xs h-[76px] flex items-end justify-center pb-1 text-[9px] font-mono font-bold transition-colors duration-75 {btnClass(
							2
						)}"
						style={isDown(2)
							? `background-color: ${themeStyles.cursorColor}; border-color: ${themeStyles.cursorColor};`
		...
		>
			<div class="absolute inset-0 flex items-center justify-center pointer-events-none">
				<span class="text-white/25 text-xs font-mono text-center px-4 whitespace-pre-line"
					>{$t('utilities.mouse.surface.hint')}</span
				>
			</div>
			{#if buttonsMask !== 0}
				<div
					class="absolute top-2 left-2 px-2 py-1 bg-black/70 border rounded-xs text-xs font-mono font-bold"
					style="color: {themeStyles.cursorColor}; border-color: {themeStyles.cursorColor};"
				>
					{$t('utilities.mouse.surface.buttons', { mask: buttonsMask })}
				</div>
			{/if}
			<div
				class="absolute bottom-2 right-2 px-2 py-1 bg-black/70 border border-white/15 rounded-xs text-[10px] font-mono text-white/50"
			>
