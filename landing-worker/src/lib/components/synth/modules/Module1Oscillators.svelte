...
<div
	class="border border-[#e5c07b]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[175px] shrink-0 xl:grow-[5]"
>
	<div
		class="flex justify-between items-center font-black text-[#e5c07b] text-xs border-b border-white/10 pb-0.5 shrink-0"
	>
	...
	<div class="flex flex-col flex-1 min-h-0 justify-center gap-1.5 py-1">
		<div class="flex gap-1.5 shrink-0">
			<div class="flex-1 min-w-0 flex flex-col gap-1.5">
				{#each [1, 2] as osc (osc)}
					...
					<div class="flex flex-col gap-1 min-w-0">
						<div class="flex items-center gap-1.5">
							<span class="text-[10px] font-black leading-none shrink-0 w-8" style="color: {color}"
								>OSC{osc}</span
							>
							<div class="flex-1 min-w-0">
								<WaveMenu
									...
									onPick={(nw: SynthWaveform) =>
										updateActiveTrack(osc === 1 ? { osc1Waveform: nw } : { osc2Waveform: nw })}
						...
						<svg
							...
							class="w-full h-8 border border-white/10 rounded-xs bg-black/40"
							aria-hidden="true"
						>
							<line
								x1="0"
			...
			<div
				class="shrink-0 flex flex-col justify-around items-center border-l border-white/10 pl-1.5"
			>
				<RotaryKnob
					label="OSC1"
					...
					color="#e5c07b"
					size={32}
					description={$t('synthPanels.osc.osc1LevelDesc')}
					reset={100}
					onChange={(v) => updateActiveTrack({ osc1Gain: v / 100 })}
				/>
				<RotaryKnob
					label="OSC2"
					...
					color="#56b6c2"
					size={32}
					description={$t('synthPanels.osc.osc2LevelDesc')}
					reset={0}
					onChange={(v) => updateActiveTrack({ osc2Gain: v / 100 })}
				/>
			</div>
		</div>
		<button
		...
		<div class="grid grid-cols-4 gap-x-0.5 gap-y-1 border-t border-white/10 pt-1.5 shrink-0">
			<RotaryKnob
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ detuneCents: v })}
			/>
			<RotaryKnob
				label="SEMI"
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ osc2Semitone: v })}
			/>
			<RotaryKnob
				label="PW"
				...
				size={32}
				reset={50}
				onChange={(v) => updateActiveTrack({ pulseWidth: v })}
			/>
			<RotaryKnob
				label="PHS"
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ phaseOffset: v })}
			/>
			<RotaryKnob
				label="SUB"
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ subOscGain: v / 100 })}
			/>
			<RotaryKnob
				label="NOISE"
			...
			<RotaryKnob
				...
				size={32}
				reset={1}
				onChange={(v) => updateActiveTrack({ noiseRetrig: v })}
			/>
			<RotaryKnob
				label="GAP"
				value={$currentTrack.noiseRetrigGap ?? 12}
				min={5}
				max={40}
				step={1}
				unit="ms"
				...
				size={32}
				reset={12}
				onChange={(v) => updateActiveTrack({ noiseRetrigGap: v })}
			/>
		</div>
	</div>
</div>
