...
<div class="border border-[#56b6c2]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[115px] shrink-0 xl:grow-[3]">
	<div class="flex justify-between items-center font-black text-[#56b6c2] text-xs border-b border-white/10 pb-0.5 shrink-0">
		...
		<div class="flex items-center gap-1.5">
			<span class="text-white/60 flex items-center" title={$t('synthPanels.rack.flowToEnvelopes')}>
			...
			<button onclick={resetRack3} title={$t('synthPanels.rack.resetHint')} class="press px-1 py-0.2 text-[9px] rounded-xs font-mono font-bold cursor-pointer transition-colors border border-white/20 text-white/60 hover:text-white hover:border-white/60">RST</button>
		</div>
	</div>
