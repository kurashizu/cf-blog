...
<div class="border border-[#56b6c2]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[115px] shrink-0 xl:grow-[3]">
	<div class="flex justify-between items-center font-black text-[#56b6c2] text-xs border-b border-white/10 pb-0.5 shrink-0">
		...
		<div class="flex items-center gap-1.5">
			<button onclick={resetRack3} title="RESET — put this rack at its neutral values, where it does nothing to the sound" class="press px-1 py-0.2 text-[9px] rounded-xs font-mono font-bold cursor-pointer transition-colors border border-white/20 text-white/40 hover:text-white hover:border-white/60">RESET</button>
	...
	<div class="grid grid-cols-12 gap-2 items-center flex-1 min-h-0 my-auto py-0.5">
		...
		<div class="col-span-7 grid grid-cols-2 gap-0.5 border-l border-white/10 pl-1.5 h-full items-center py-0.5">
			...
			<RotaryKnob
				...
				color="#98c379"
				size={32}
				reset={0}	onChange={(v) => updateActiveTrack({ filterEnvAmount: v / 100, envFilterMod: Math.max(0, v / 100) })}
			/>
		</div>
	</div>
</div>
