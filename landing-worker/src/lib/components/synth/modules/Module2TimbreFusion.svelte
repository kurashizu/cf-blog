...
<div
	class="border border-[#c678dd]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[115px] shrink-0 xl:grow-[3]"
>
	<div
	...
	<div class="grid grid-cols-12 gap-2 items-center flex-1 min-h-0 my-auto py-0.5">
		<div class="col-span-5 grid grid-cols-2 grid-rows-2 gap-1 h-full py-0.5">
			{#each BLEND_MODES as mode (mode)}
				<button
					...
					class="press h-full w-full flex items-center justify-center text-xs border rounded-xs font-black cursor-pointer transition-colors leading-none text-center {$currentTrack.blendMode ===
					mode
		...
		>
			<RotaryKnob
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ morphAmount: v / 100 })}
			/>
			<RotaryKnob
				label="RATIO"
				...
				size={32}
				reset={1}
				onChange={(v) => updateActiveTrack({ osc2Ratio: v })}
			/>
			<RotaryKnob
				label="X-FADE"
				...
				size={32}
				reset={50}
				onChange={(v) => updateActiveTrack({ xfade: v / 100 })}
			/>
			<RotaryKnob
				label="GLIDE"
				...
				size={32}
				reset={0}
				onChange={(v) => updateActiveTrack({ glideTime: v })}
			/>
		</div>
	</div>
</div>
