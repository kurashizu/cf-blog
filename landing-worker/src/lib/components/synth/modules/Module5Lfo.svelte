<script lang="ts">
	...
	const LFO_LABELS: Record<LfoWaveform, string> = {
		sine: 'SIN',
		triangle: 'TRI',
		square: 'SQR',
		sawtooth: 'SAW'
...
<div
	class="xl:col-span-1 border border-[#c678dd]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[155px] xl:min-h-[128px] shrink-0"
>
	<div
	...
	<div class="grid grid-cols-12 gap-1 items-center flex-1 min-h-0 my-auto">
		<div
			class="col-span-4 flex flex-col justify-between items-center gap-1 border-r border-white/10 pr-1 h-full py-0.5"
		>
			<div class="grid grid-cols-2 gap-0.5 w-full">
				{#each LFO_WAVES as w (w)}
					<button
						...
						class="press py-1 text-[10px] sm:text-xs border rounded-xs font-black cursor-pointer leading-none text-center transition-colors {$currentTrack.lfoWaveform ===
						w
			...
			<RotaryKnob
				...
				color="#c678dd"
				size={40}
				description={$t('synthPanels.lfo.rateDesc')}
				reset={5}
				onChange={(v) => updateActiveTrack({ lfoRate: v })}
			/>
		</div>
			...
			<div class="grid grid-cols-6 gap-0.5 items-center">
				<div class="col-span-2 flex justify-center">
					<RotaryKnob
						...
						color="#e5c07b"
						size={40}
						description={$t('synthPanels.lfo.pitchDesc')}
						reset={0}
						onChange={(v) => updateActiveTrack({ lfoPitchAmt: v / 100 })}
					/>
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob
						...
						color="#56b6c2"
						size={40}
						description={$t('synthPanels.lfo.cutoffDesc')}
						reset={0}
						onChange={(v) => updateActiveTrack({ lfoCutoffAmt: v / 100 })}
					/>
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob
						...
						color="#d19a66"
						size={40}
						description={$t('synthPanels.lfo.ampDesc')}
						reset={0}
						onChange={(v) => updateActiveTrack({ lfoAmpAmt: v / 100 })}
					/>
				</div>
			</div>
			...
			<div class="grid grid-cols-6 gap-0.5 items-center">
				...
				<div class="col-span-2 flex justify-center">
					<RotaryKnob
						...
						color="#98c379"
						size={40}
						description={$t('synthPanels.lfo.panDesc')}
						reset={0}
						onChange={(v) => updateActiveTrack({ lfoPanAmt: v / 100 })}
					/>
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob
						...
						color="#c678dd"
						size={40}
						description={$t('synthPanels.lfo.fadeDesc')}
						reset={0}
						onChange={(v) => updateActiveTrack({ lfoFadeTime: v })}
					/>
				</div>
				<div class="col-span-1"></div>
			</div>
