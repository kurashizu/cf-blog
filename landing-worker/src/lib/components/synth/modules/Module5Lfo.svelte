...
<div class="xl:col-span-5 border border-[#c678dd]/40 p-1.5 bg-black/60 rounded-xs flex flex-col justify-between min-h-[155px] shrink-0">
	...
	<div class="grid grid-cols-12 gap-1 items-center flex-1 min-h-0 my-auto">
		<div class="col-span-4 flex flex-col justify-between items-center gap-1 border-r border-white/10 pr-1 h-full py-0.5">
			...
			<RotaryKnob label="RATE" value={$currentTrack.lfoRate} min={0.1} max={20} step={0.2} unit="Hz" color="#c678dd" size={40} reset={5} onChange={(v) => updateActiveTrack({ lfoRate: v })} />
		</div>
		...
		<div class="col-span-8 flex flex-col justify-around h-full py-0.5 my-auto">
			<div class="grid grid-cols-6 gap-0.5 items-center">
				<div class="col-span-2 flex justify-center">
					<RotaryKnob label="PITCH" value={Math.round(($currentTrack.lfoPitchAmt ?? 0) * 100)} min={0} max={100} step={5} unit="%" color="#e5c07b" size={40} reset={0} onChange={(v) => updateActiveTrack({ lfoPitchAmt: v / 100 })} />
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob label="CUTOFF" value={Math.round(($currentTrack.lfoCutoffAmt ?? 0) * 100)} min={0} max={100} step={5} unit="%" color="#56b6c2" size={40} reset={0} onChange={(v) => updateActiveTrack({ lfoCutoffAmt: v / 100 })} />
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob label="AMP" value={Math.round(($currentTrack.lfoAmpAmt ?? 0) * 100)} min={0} max={100} step={5} unit="%" color="#d19a66" size={40} reset={0} onChange={(v) => updateActiveTrack({ lfoAmpAmt: v / 100 })} />
				</div>
			</div>
			...
			<div class="grid grid-cols-6 gap-0.5 items-center">
				...
				<div class="col-span-2 flex justify-center">
					<RotaryKnob label="PAN" value={Math.round(($currentTrack.lfoPanAmt ?? 0) * 100)} min={0} max={100} step={5} unit="%" color="#98c379" size={40} reset={0} onChange={(v) => updateActiveTrack({ lfoPanAmt: v / 100 })} />
				</div>
				<div class="col-span-2 flex justify-center">
					<RotaryKnob label="FADE" value={$currentTrack.lfoFadeTime ?? 0} min={0} max={2000} step={50} unit="ms" color="#c678dd" size={40} reset={0} onChange={(v) => updateActiveTrack({ lfoFadeTime: v })} />
				</div>
				<div class="col-span-1"></div>
			</div>
		</div>
