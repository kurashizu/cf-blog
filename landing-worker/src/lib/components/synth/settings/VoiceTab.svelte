...
<div class="space-y-4">
	...
	<div class="border border-white/10 bg-black/40 rounded-xs p-3 space-y-3">
		<div class="flex items-center justify-between border-b border-white/10 pb-1">
			...
			<span class="text-white/60 text-[10px]">{$t('synthPanels.voice.tuningSubtitle')}</span>
		</div>

		<div class="flex items-center justify-between pt-1">
			<div>
				<p class="text-white/80 font-bold text-sm">{$masterTuningSetting.toFixed(1)} Hz</p>
				<p class="text-white/60 text-[10px]">{$t('synthPanels.voice.tuningDesc')}</p>
			</div>
	...
	<div class="border border-white/10 bg-black/40 rounded-xs p-3 space-y-3">
		<div class="flex items-center justify-between border-b border-white/10 pb-1">
			...
			<span class="text-white/60 text-[10px]">{$t('synthPanels.voice.polyphonySubtitle')}</span>
		</div>

		<div class="flex items-center justify-between pt-1">
			<div>
				<p class="text-white/80 font-bold">{$t('synthPanels.voice.maxVoicesLabel', { count: $maxPolyphonySetting })}</p>
				<p class="text-white/60 text-[10px]">{$t('synthPanels.voice.polyphonyDesc')}</p>
			</div>
	...
	<div class="border border-white/10 bg-black/40 rounded-xs p-3 space-y-2.5">
		<div class="flex items-center justify-between border-b border-white/10 pb-1">
			...
			<span class="text-white/60 text-[10px]">{$t('synthPanels.voice.stealingSubtitle')}</span>
		</div>
		...
		<div class="grid grid-cols-3 gap-2 pt-1">
			{#each VOICE_STEALING_MODES as item (item.id)}
				...
				>
					...
					<div class="text-[9px] {$voiceStealingSetting === item.id ? 'text-black/80' : 'text-white/60'}">{$t(item.descKey)}</div>
				</button>
			{/each}
		</div>
	</div>
