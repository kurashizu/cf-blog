...
{#if $isSynthSettingsOpen}
	...
	<div
		class="fixed inset-0 z-[100] flex items-center justify-center p-3 bg-black/80 backdrop-blur-xs animate-fadeIn select-none"
		onclick={close}
	>
		<!-- svelte-ignore a11y_click_events_have_key_events -->
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		...
		>
			...
			<div class="flex items-center justify-between px-3 py-2 bg-black/60 border-b border-white/10 shrink-0">
				...
				<button
					onclick={close}
					class="w-6 h-6 border border-white/20 hover:border-[#e06c75] hover:bg-[#e06c75]/20 text-white/60 hover:text-[#e06c75] rounded-xs flex items-center justify-center text-xs font-black cursor-pointer transition-colors"
			...
			<div class="flex items-center gap-1 px-3 py-1.5 bg-black/40 border-b border-white/10 shrink-0 text-xs">
				{#each TABS as tab (tab.id)}
					<button
						...
						class="px-2.5 py-1 rounded-xs border font-black cursor-pointer transition-all {$synthSettingsTab === tab.id
			...
			<div class="flex items-center justify-between px-4 py-2 bg-black/60 border-t border-white/10 shrink-0 text-xs">
				...
				<button onclick={close} class="px-4 py-1 bg-[#e5c07b] text-black font-black rounded-xs hover:opacity-90 cursor-pointer shadow-xs">
					DONE
				</button>
			</div>
		</div>
