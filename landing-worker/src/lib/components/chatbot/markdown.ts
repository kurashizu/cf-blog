...
function inline(s: string): string {
	/* `code` is lifted out before the other patterns run, not merely matched
	   first: these are independent passes over the whole string, so a span
	   left inline would still have its contents rewritten by the emphasis
	   and link passes that follow, and `**x**` inside backticks would come
	   back bold.

	   The marker is NUL-delimited, the same trick highlight() uses below: a
	   model reply is text, so it cannot contain a NUL to forge one with. */
	const codes: string[] = [];
