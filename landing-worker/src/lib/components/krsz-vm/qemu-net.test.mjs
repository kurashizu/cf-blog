...
globalThis.WebSocket = class {
	constructor(url) {
		this.url = url;
		ws = this;
		sent.push(['ws-open', url]);
		queueMicrotask(() => setTimeout(() => this.onopen?.(), 0));
	}
	send(b) {
		sent.push(['ws-send', new Uint8Array(b)]);
	}
	close() {}
};
...
function frame(dstMac, type, payload) {
	...
	new DataView(w.buffer).setUint32(0, f.length, false);
	w.set(f, 4);
	return w;
}
const parse = (w) => {
	const len = new DataView(w.buffer, w.byteOffset, 4).getUint32(0, false);
	return { len, eth: w.subarray(4, 4 + len) };
};
...
if (out.length) {
	...
	const p = eth.subarray(14);
	check('ARP is a reply', ((p[6] << 8) | p[7]) === 2);
	check(
		'ARP carries the gateway MAC',
		[...p.subarray(8, 14)].join() === '82,85,10,0,2,2',
		[...p.subarray(8, 14)].join()
	);
...
if (out.length) {
	const { eth } = parse(out[0]);
	check(
		'OFFER goes to the broadcast MAC',
		[...eth.subarray(0, 6)].every((b) => b === 0xff)
	);
	const bootp = eth.subarray(14 + 20 + 8);
	check(
		'OFFER yiaddr is 10.0.2.15',
		[...bootp.subarray(16, 20)].join() === '10,0,2,15',
		[...bootp.subarray(16, 20)].join()
	);
...
if (out.length) {
	const { eth } = parse(out[0]);
	const ip = eth.subarray(14);
	let sum = 0;
	for (let i = 0; i < 20; i += 2) sum += (ip[i] << 8) | ip[i + 1];
...
function tcp(flags, seq, ack, dport, payload = new Uint8Array(0)) {
	...
	v.setUint16(0, 45000, false);
	v.setUint16(2, dport, false);
	v.setUint32(4, seq, false);
	v.setUint32(8, ack, false);
	seg[12] = 5 << 4;
	seg[13] = flags;
...
if (wispFrames.length) {
	...
	check(
		'CONNECT names the host',
		new TextDecoder().decode(b.subarray(8)) === '93.184.216.34',
		new TextDecoder().decode(b.subarray(8))
	);
}
...
check(
	'guest payload reached the relay',
	dataFrames.some(
		(f) => f[1][0] === 0x02 && new TextDecoder().decode(f[1].subarray(5)).startsWith('GET /')
	)
);
check(
	'and was acked',
	out.some((w) => {
		const { eth } = parse(w);
		return (eth.subarray(14 + 20)[13] & 0x10) !== 0;
	})
);
...
// ---- the return path: bytes from the relay reach the guest as TCP segments
{
	...
	if (out.length) {
		...
		const seg = eth.subarray(14 + 20);
		check(
			'delivered with PSH|ACK',
			(seg[13] & 0x08) !== 0 && (seg[13] & 0x10) !== 0,
			'flags=0x' + seg[13].toString(16)
		);
		check(
			'carries the body',
			new TextDecoder().decode(seg.subarray(20)).startsWith('HTTP/1.0 200')
		);
	...
	check(
		'upstream close becomes a FIN',
		out.some((w) => (parse(w).eth.subarray(14 + 20)[13] & 0x01) !== 0)
	);
}
