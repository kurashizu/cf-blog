...
// Vendored from v86, src/browser/fake_network.js, at b0d8f2c.
// Copyright (c) 2012, The v86 contributors. BSD-2-Clause; see v86's LICENSE.
//
// This is the gateway the other machine on this page has been using all along:
// ARP, DHCP, ICMP, a DNS resolver over DoH, and a TCP implementation with a
// real state machine. The x86-64 machine needs the same thing, and QEMU does
// not bring one -- libslirp is not in this wasm build. Rather than keep growing
// a second implementation beside a working one, this is the working one.
//
// Changes from upstream, kept to the minimum so it stays diffable:
//   - the three imports become local stubs (v86's logging and its hex helper)
//   - nothing else

const LOG_FETCH = 0;
const h = (n, len = 0) => "0x" + n.toString(16).toUpperCase().padStart(len, "0");
const dbg_log = () => {};
const dbg_assert = (cond, msg) => { if(!cond) throw new Error(msg || "assertion failed"); };
