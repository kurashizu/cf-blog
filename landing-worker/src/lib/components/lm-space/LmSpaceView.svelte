...
<div class="flex-1 min-h-0 flex flex-col">
	<div class="flex items-center gap-1.5 pb-1.5 shrink-0" data-tour="lms-modes">
		<span class="text-[10px] font-mono font-bold text-white/40 uppercase tracking-widest mr-0.5"
			>{$t('lmspace.modes.viewAs')}</span
		>
		{#each MODES as { k, label } (k)}
			<button
				...
				class="press px-2 py-1 border rounded-xs text-xs font-bold cursor-pointer transition-colors {mode ===
				k
	...
	{#if mobile && mobileNoteOpen}
		<div
			class="shrink-0 mb-1.5 px-2.5 py-2 border border-[#e5c07b]/50 bg-[#e5c07b]/10 rounded-xs text-xs font-mono text-[#e5c07b] flex items-start gap-2"
			transition:fade={{ duration: 160 }}
		>
			<span class="flex-1">
				<b>{$t('lmspace.mobile.title')}</b>
				{$t('lmspace.mobile.body')}
			</span>
			<button
				onclick={() => {
	...
	<div
		class="lmspace relative flex-1 min-h-0 overflow-hidden border border-white/10 rounded-xs"
	...
	>
		...
		<div class="hud" id="topbar">
			<div>
				...
				<div class="sub">
					...
					<span class="hint-tip" title={$t('lmspace.hud.sourceTip')}
						>{$t('lmspace.hud.source')} &#9432;</span
					>
				</div>
			</div>
			<div id="ctl" data-tour="lms-ctl">
				<div class="grp" style="--g:#56b6c2">
					...
					<div class="gbtns">
						...
						<span class="cyc" id="view-cyc">
							<button class="carrow" id="view-prev" title={$t('lmspace.hud.prevView')}
								>&#9664;</button
							>
							<button class="cval" id="view-val" title={$t('lmspace.hud.viewTip')}></button>
							<button class="carrow" id="view-next" title={$t('lmspace.hud.nextView')}
								>&#9654;</button
							>
						</span>
					</div>
				...
				<div class="grp" style="--g:#61afef">
					...
					<div class="gbtns">
						...
						<span class="cyc" id="proj-cyc">
							<button class="carrow" id="proj-prev" title={$t('lmspace.hud.prevProjection')}
								>&#9664;</button
							>
							<button class="cval" id="proj-val" title={$t('lmspace.hud.projectionTip')}></button>
							<button class="carrow" id="proj-next" title={$t('lmspace.hud.nextProjection')}
								>&#9654;</button
							>
						</span>
						<button class="btn" id="vp-cycle" title={$t('lmspace.hud.vpCycleTip')}
							>$ &times; I</button
						>
					</div>
				</div>
				<div class="grp" style="--g:#d19a66">
					...
					<div class="gbtns">
						<button class="btn" id="g-start" title={$t('lmspace.hud.simulateTip')}
							>{$t('lmspace.hud.simulate')}</button
						>
						<button class="btn on" id="g-hull" title={$t('lmspace.hud.linkTip')}
							>{$t('lmspace.hud.link')}</button
						>
					</div>
				</div>
				<div class="grp" style="--g:#98c379">
					...
					<div class="gbtns">
						<button class="btn" id="pareto-toggle" title={$t('lmspace.hud.frontierTip')}
							>{$t('lmspace.hud.frontier')}</button
						>
					</div>
				</div>
			</div>
		...
		<div id="modes" data-tour="lms-axes">
			<div class="panel">
				...
				<div
					id="axinfo"
					style="font-size:1.1667rem;line-height:1.75;color:rgba(255,255,255,.55)"
				></div>
			</div>
		</div>
		...
		<div id="hint">
			<span title={$t('lmspace.hint.full')}>
				<kbd>WASD</kbd>
				{$t('lmspace.hint.fly')} &middot; <kbd>R</kbd>
				{$t('lmspace.hint.reset')} &middot; {$t('lmspace.hint.controls')} &#9432;
			</span>
		</div>
		...
		<div id="range"></div>
		<div id="outwarn">
			{$t('lmspace.outwarn.outside')} &middot; <kbd>R</kbd>
			{$t('lmspace.outwarn.toReturn')}
		</div>
		<div id="mission"></div>
		...
		<div id="racectl">
			...
			<button type="button" id="race-back" title={$t('lmspace.race.stepBack')}
				>&#9664;&#9664;</button
			>
		...
		{#if booting}
			<div
				class="absolute inset-0 flex items-center justify-center text-xs font-mono text-[#56b6c2] pointer-events-none"
...
<style>
   ...
   prefixed with .lmspace, which keeps it off the rest of the app. */
	...
	:global(.lmspace) *,
	:global(.lmspace) *::before,
	:global(.lmspace) *::after {
	...
	:global(.lmspace) {
       ...
       the view had never followed that setting at all before. */
		font-family:
			'Jelly Pixel', 'KRSZ Box', ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
		font-size: 1rem;
		line-height: 2;
		overflow: hidden;
	}
   ...
   title is readable, a clipped button is not. */
	:global(.lmspace #topbar > div:first-child) {
		min-width: 0;
		overflow: hidden;
	}
	:global(.lmspace #ctl) {
	...
	:global(.lmspace .btn:hover) {
		border-color: var(--g, rgba(255, 255, 255, 0.5));
		color: #fff;
	}
	:global(.lmspace .btn.on) {
		color: #0a0b0d;
		background: var(--g, #fff);
		border-color: var(--g, #fff);
	}
	:global(.lmspace .btn.inert) {
		opacity: 0.28;
		cursor: default;
		pointer-events: none;
	}
	:global(.lmspace #vp-cycle) {
		min-width: 8ch;
		text-align: center;
	}
   ...
   closed on a phone-sized one. */
	...
	:global(.lmspace .pcaret) {
		display: inline;
		transition: transform 0.15s;
	}
	:global(.lmspace .collapsed .pcaret) {
	...
	:global(.lmspace #legend.collapsed #fcreators) {
		display: none;
	}
   ...
   buttons that light up filled rather than just changing text colour. */
	...
	:global(.lmspace .lg.mute) {
		opacity: 0.4;
	}
	:global(.lmspace .dot) {
		display: none;
	}
	:global(.lmspace .lgname) {
		flex: 1;
		overflow: hidden;
	...
	:global(.lmspace .lgm:hover),
	:global(.lmspace .lgs:hover) {
		color: #fff;
		border-color: rgba(255, 255, 255, 0.5);
	}
	:global(.lmspace .lgm.on) {
		background: #e06c75;
		border-color: #e06c75;
		color: #0a0b0d;
		font-weight: 900;
	}
	:global(.lmspace .lgs.on) {
		background: #e5c07b;
		border-color: #e5c07b;
		color: #0a0b0d;
		font-weight: 900;
	}
	:global(.lmspace .fcount) {
		font-size: 0.9167rem;
		color: rgba(255, 255, 255, 0.4);
		margin-bottom: 7px;
	}
	:global(.lmspace #franges) {
		display: flex;
		flex-direction: column;
		gap: 11px;
		margin-bottom: 8px;
   ...
   native input so two handles can share one track. */
	:global(.lmspace .fslider) {
		position: relative;
		height: 14px;
		cursor: ew-resize;
		touch-action: none;
	}
	:global(.lmspace .ftrack) {
		position: absolute;
		left: 0;
		right: 0;
		top: 6px;
	...
	:global(.lmspace .fhandle:hover),
	:global(.lmspace .fhandle:active) {
		filter: brightness(1.2);
	}
	:global(.lmspace .fgrip) {
		width: 1px;
		height: 6px;
   ...
   thing in the scene. */
	...
	:global(.lmspace #card .nm) {
		font-size: 0.9167rem;
		font-weight: 900;
		color: var(--cyan);
		line-height: 1.35;
	}
	:global(.lmspace #card .cr) {
		font-size: 0.9167rem;
		color: rgba(255, 255, 255, 0.4);
		margin-top: 2px;
	}
	:global(.lmspace #card .qwarn) {
		font-size: 1rem;
		color: var(--orange);
		margin-top: 3px;
		line-height: 1.4;
	...
	:global(.lmspace .x:hover) {
		color: #fff;
	}
	:global(.lmspace #mission) {
		position: absolute;
		top: 52px;
		left: 50%;
	...
	:global(.lmspace #mission .q) {
		font-size: 0.9167rem;
		font-weight: 700;
		color: var(--yellow);
	}
	:global(.lmspace #mission .m) {
		font-size: 0.9167rem;
		color: rgba(255, 255, 255, 0.5);
		margin-top: 3px;
	}
	:global(.lmspace #mission .res) {
		font-size: 1rem;
		margin-top: 5px;
		font-weight: 700;
	}
	...
	:global(.lmspace .fps-n) {
		font-weight: 700;
	}
	:global(.lmspace .fps-l) {
		color: rgba(255, 255, 255, 0.35);
	}
	:global(.lmspace #crosshair) {
		position: absolute;
		left: 50%;
		top: 50%;
		z-index: 25;
	...
	:global(.lmspace #crosshair:before),
	:global(.lmspace #crosshair:after) {
		content: '';
		position: absolute;
		background: rgba(255, 255, 255, 0.5);
		transition: background 0.1s;
	}
	:global(.lmspace #crosshair.hot:before),
	:global(.lmspace #crosshair.hot:after) {
		background: var(--green);
	}
	:global(.lmspace #crosshair.hot) {
	...
	:global(.lmspace #racectl.show) {
		display: flex;
	}
	:global(.lmspace #racectl button) {
		flex: 1;
		padding: 4px 0;
		border: 1px solid rgba(255, 255, 255, 0.18);
		border-radius: 2px;
	...
	:global(.lmspace #racectl button:hover) {
		border-color: var(--purple);
		color: #fff;
	}
	:global(.lmspace #racectl button.on) {
		color: #0a0b0d;
	...
	:global(.lmspace .rrow.me) {
		background: rgba(198, 120, 221, 0.16);
		outline: 1px solid rgba(198, 120, 221, 0.4);
		border-radius: 2px;
		color: #fff;
	}
	:global(.lmspace .rrow .rk) {
		min-width: 3.2ch;
		text-align: right;
		color: rgba(255, 255, 255, 0.3);
		flex: 0 0 auto;
	...
	:global(.lmspace .rrow .rv) {
		font-weight: 700;
		flex: 0 0 auto;
		font-variant-numeric: tabular-nums;
	}
	:global(.lmspace .rhint) {
		font-size: 1rem;
		color: rgba(255, 255, 255, 0.35);
		margin-top: 5px;
		padding-top: 4px;
	...
	:global(.lmspace .gcl:first-of-type) {
		border-top: 0;
	}
	:global(.lmspace .gcl-h) {
		font-size: 0.9167rem;
		color: rgba(255, 255, 255, 0.72);
	}
	:global(.lmspace .gcl-m) {
		font-size: 1rem;
		color: rgba(255, 255, 255, 0.42);
		margin-top: 1px;
	}
	:global(.lmspace #range) {
		position: absolute;
		left: 50%;
		top: calc(50% + 22px);
		transform: translateX(-50%);
	...
	:global(.lmspace #range .rg-lab) {
		color: rgba(255, 255, 255, 0.3);
	}
	:global(.lmspace #range .rg-num) {
		color: var(--cyan);
		font-weight: 700;
	}
	:global(.lmspace #range.mid .rg-num) {
		color: var(--yellow);
	}
	:global(.lmspace #range.far .rg-num) {
		color: var(--red);
	}
	:global(.lmspace #range .rg-warn) {
		color: var(--red);
	}
	:global(.lmspace #outwarn) {
		position: absolute;
		left: 50%;
		top: calc(50% - 30px);
	...
	:global(.lmspace #outwarn.show) {
		opacity: 1;
		transform: translate(-50%, 0);
	}
	:global(.lmspace #outwarn kbd) {
		border: none;
		padding: 0;
		font-size: 0.75rem;
		color: var(--red);
   ...
   re-fits itself when the stage is laid out again. */
	...
	:global(.lmspace #tip.show) {
		opacity: 1;
		transform: none;
	}
	:global(.lmspace #tip::after) {
		content: '';
		position: absolute;
		left: var(--arrow, 20px);
		width: 8px;
	...
	:global(.lmspace #tip[data-side='below']::after) {
		top: -5px;
		border-right: none;
		border-bottom: none;
	}
	:global(.lmspace #tip[data-side='above']::after) {
		bottom: -5px;
		border-left: none;
		border-top: none;
	}
	:global(.lmspace .has-tip),
	:global(.lmspace .hint-tip) {
		cursor: help;
	}
   ...
   needs the same .cyc font metrics restated at this smaller size. */
	:global(.lmspace #rfield .cyc) {
		font-size: 1.1667rem;
	}
	:global(.lmspace #rfield .cval) {
		font-size: 0.9167rem;
	}
	:global(.lmspace #boot) {
		position: absolute;
	...
	:global(.lmspace ::-webkit-scrollbar) {
		width: 7px;
		height: 7px;
	}
	:global(.lmspace ::-webkit-scrollbar-thumb) {
