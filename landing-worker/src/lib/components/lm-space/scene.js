...
export async function mountLmSpace(root, payload) {
   ...
   out again. */
	...
	const posSpace = (m, idx) => {
		...
		if (m.q === 'B') {
			...
			return new THREE.Vector3((norm(lg(m.p), pLo, pHi) - 0.5) * 2 * S, y, wanderAxis(idx, 0, 0));
		}
		if (m.q === 'D') {
	...
	function wanderAxis(idx, salt, time) {
		...
		const rate = 0.06 + Math.abs(Math.sin(seed * 1.7)) * 0.1;
	...
	function driftPos(idx, time) {
		...
		if (lead !== idx) {
			...
			return p
				.clone()
				.add(
					new THREE.Vector3(Math.sin(k * 2.1) * 7, Math.cos(k * 1.7) * 5, Math.sin(k * 3.3) * 7)
				);
		}
		...
		const reach = BOX_REACH * (1.1 + (((idx * 5) % 8) / 8) * 0.24);
		let px = (x / len) * reach;
		...
		const EX_MIN = -(WANDER_MIN + WANDER_SPAN) - 12,
			EX_MAX = S + 12;
		const inside =
			px > EX_MIN && px < EX_MAX && pz > EX_MIN && pz < EX_MAX && py > -S - 12 && py < S + 12;
		if (inside) {
   ...
   annexes, which put the actual data in the middle distance. */
	...
	function updateSkins() {
		for (const m of skins) {
			...
			const c =
				axis === 'x' ? camera.position.x : axis === 'y' ? camera.position.y : camera.position.z;
	...
	function ticks(axis, count) {
		...
		for (let k = 0; k <= count; k++) {
			...
			if (axis === 'x') {
				pts.push(V(t, -1, -1), V(t, -1 + L, -1));
				pts.push(V(t, -1, -1), V(t, -1, -1 + L));
			}
			if (axis === 'y') {
				pts.push(V(-1, t, -1), V(-1 + L, t, -1));
				pts.push(V(-1, t, -1), V(-1, t, -1 + L));
			}
			if (axis === 'z') {
				pts.push(V(-1, -1, t), V(-1 + L, -1, t));
				pts.push(V(-1, -1, t), V(-1, -1 + L, t));
			}
		}
	...
	const shadows = new THREE.Points(
		shadowGeo,
		new THREE.ShaderMaterial({
			transparent: true,
			depthWrite: false,
	...
	function buildAnnexes() {
		...
		const bTag = tag(
			tr('lmspace.zone.speedUnknown'),
			new THREE.Vector3(0, S + 6, (zA + zB) / 2),
			'rgba(209,154,102,.85)',
			'tag zone',
			tr('lmspace.zone.speedUnknownTip')
		);
		annexGroup.add(bTag);

		// D: only intelligence is real, so the annexe is a corner block.
		annexGroup.add(boxWire([zB - 6, -S, zB - 6], [zA + 6, S, zA + 6], GREY, 0.3));
		annexGroup.add(
			tag(
				tr('lmspace.zone.priceSpeedUnknown'),
				new THREE.Vector3((zA + zB) / 2, S + 6, (zA + zB) / 2),
				'rgba(124,135,148,.8)',
				'tag zone',
	...
	scene.add(spineGroup);
	{
		...
		const shaft = lineSet(
			[new THREE.Vector3(cx, -S, cz), new THREE.Vector3(cx, S, cz)],
			AX.y,
			0.95
		);
		...
		for (const v of [10, 20, 30, 40, 50, 60]) {
			...
			const x0 = -S - ANNEX_GAP - ANNEX_D - 6,
				x1 = S;
			const z0 = -S - ANNEX_GAP - ANNEX_D - 6,
				z1 = S;
			const pts = [];
		...
		spineGroup.add(
			tag(
				tr('lmspace.axis.intelligence'),
				new THREE.Vector3(cx, S + 12, cz),
				'rgba(86,182,194,.95)',
				'tag axmain',
				tr('lmspace.axis.intelligenceTip')
			)
		);
	}
	...
	scene.add(timeSpineGroup);
	{
		...
		const floorPts = [],
			ceilPts = [];
		for (let k = 0; k < WALL_SEGS; k++) {
			for (const [dk, arr, y] of [
				[0, floorPts, -S],
				[1, ceilPts, S]
			]) {
				const t0 = k / WALL_SEGS,
					t1 = (k + 1) / WALL_SEGS;
				const a0 = t0 * SPIRAL_TURNS * Math.PI * 2,
					a1 = t1 * SPIRAL_TURNS * Math.PI * 2;
				const r0 = t0 * SPIRAL_R,
					r1 = t1 * SPIRAL_R;
		...
		timeSpineGroup.add(
			tag(
				...
				'rgba(86,182,194,.95)',
				'tag axmain',
				tr('lmspace.axis.intelligenceSpiralTip')
			)
		);
		timeSpineGroup.add(
			tag(
				tr('lmspace.axis.earliest'),
				new THREE.Vector3(cx, -S - 10, cz),
				'rgba(229,192,123,.85)',
				'tag axmain',
				tr('lmspace.axis.earliestTip')
			)
		);
	}
	...
	function buildAxisLabels(mode) {
		...
		if (mode === 'space') {
			...
			for (const v of niceTicks(pLo, pHi, 5)) {
				mk(
					v === 0 ? tr('lmspace.axis.free') : '$' + fmtTick(v),
					new THREE.Vector3((norm(lg(v), pLo, pHi) - 0.5) * 2 * S, -S - 7, -S),
					'rgba(229,192,123,.55)'
				);
			}
			for (const v of niceTicks(sLo, sHi, 5)) {
				mk(
					fmtTick(v),
					new THREE.Vector3(-S - 4, -S - 7, (norm(lg(v), sLo, sHi) - 0.5) * 2 * S),
					'rgba(97,175,239,.55)'
				);
			}
		} else {
	...
	const RADIUS_FIELDS = {
		...
		coding: {
			get: (m) => m.cd,
			label: 'lmspace.field.coding',
			ext: ext((m) => (m.cd == null ? NaN : m.cd))
		}
	};
	...
	const RANGE_FIELDS = {
		price: {
			get: (m) => m.p,
			lo: Math.pow(10, pLo) - 0.05,
			hi: Math.pow(10, pHi) - 0.05,
			log: true
		},
		intel: { get: (m) => m.i, lo: iLo, hi: iHi, log: false },
		speed: {
			get: (m) => m.sp,
			lo: Math.pow(10, sLo) - 0.05,
			hi: Math.pow(10, sHi) - 0.05,
			log: true
		},
	...
	const spinAxis = [];
	{
		const ts = MODELS.map((m) => (m.t == null ? NaN : m.t))
			.filter(Number.isFinite)
			.sort((a, b) => a - b);
					...
					: 1 -
						THREE.MathUtils.clamp(
							(Math.log(Math.max(t, 0.05)) - Math.log(tLo)) /
								Math.max(Math.log(tHi) - Math.log(tLo), 0.001),
							0,
							1
						);
   ...
   camera moves, the marks change while flying. */
	...
	function writeInstances() {
		...
		for (let i = 0; i < N; i++) {
			...
			if (!gravityOn && view === 'space' && q !== 'A') {
				...
				if (q === 'X') {
					...
					if (q === 'B') cur[i].z = wanderAxis(i, 0, driftClock);
						...
						cur[i].x = wanderAxis(i, 1, driftClock);
						cur[i].z = wanderAxis(i, 2, driftClock);
					}
				}
			} else if (!gravityOn) {
			...
			let r = radiusOf(m) * (off ? 0.35 : 1) * (1 + e * 0.55 + e * 0.1 * Math.sin(nowMs / 320));
	...
	for (let k = 0; k < LABEL_POOL; k++) {
		...
		el.className = 'ntag';
		el.innerHTML =
			'<span class="nt-row"><span class="nt-line"></span>' + '<span class="nt-txt"></span></span>';
	...
	function declutterLabels() {
		...
		for (const [dist, i, sx, sy] of cand) {
			...
			for (const p of placed) {
				if (Math.abs(p[0] - lx) < LBL_W && Math.abs(p[1] - ly) < LBL_H) {
					clash = true;
					break;
				}
			}
	...
	for (const g of FAMILIES) {
		g.forEach((i, k) => {
			famRank[i] = g.length > 1 ? k / (g.length - 1) : 1;
		});
	}
	...
	onDoc('mousemove', (e) => {
		if (locked && !raceOn) applyLook(e.movementX, e.movementY);
	});
	...
	function updateRange() {
		...
		if (shown !== lastRangeShown || exact !== lastRangeExact) {
			lastRangeShown = shown;
			lastRangeExact = exact;
			rangeEl.innerHTML =
		...
		rangeEl.style.opacity = locked || vel.lengthSq() > 4 || out > 1.35 || exact ? '1' : '0';
	...
	function hoverTest(e) {
		...
		if (i !== hoverIdx) {
			hoverIdx = i;
			cv.style.cursor = i === -1 ? 'grab' : 'pointer';
		}
	}
	...
	const money = (v) =>
		v == null
			? '—'
			: v === 0
				? tr('lmspace.axis.free')
				: '$' + (+v).toFixed(v < 1 ? 2 : v < 10 ? 2 : 0);
	const esc = (s) =>
		String(s).replace(
			/[&<>"]/g,
			(c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]
	...
	const kv = (k, v, c) =>
		`<div class="kv"><div class="k">${k}</div><div class="v" style="color:${c}">${v}</div></div>`;
	function rank(m, keyName, lowBest) {
		if (m[keyName] == null) return '—';
		const vs = MODELS.filter((x) => !isOff(x))
			.map((x) => x[keyName])
			.filter((v) => v != null);
        ...
        <div style="min-width:0">
          ...
          ${
						m.q === 'A'
							...
							: `<div class="qwarn">${
									m.q === 'B'
										? tr('lmspace.card.qwarnB')
										: m.q === 'D'
											? tr('lmspace.card.qwarnD')
	...
	function onFilterChanged() {
		...
		if (gravityOn) {
			gravSettled = false;
			gravCalm = 0;
			gravAnneal = Math.min(gravAnneal, 0.55);
			lastClusterSig = '';
		}
		updateFilterCount();
		buildParetoViz();
	...
	function wireRangeRow(row) {
		...
		const loEl = track.querySelector('.flo'),
			hiEl = track.querySelector('.fhi');
		const fill = track.querySelector('.ffill');
		const cfg = RANGE_FIELDS[key];
		const slLo = toSlider(key, cfg.lo),
			slHi = toSlider(key, cfg.hi);
		let vLo = slLo,
			vHi = slHi;

		const pctOf = (v) => (v - slLo) / (slHi - slLo);
		const paint = () => {
			const loP = pctOf(vLo) * 100,
				hiP = pctOf(vHi) * 100;
			loEl.style.left = loP + '%';
			hiEl.style.left = hiP + '%';
			fill.style.left = loP + '%';
		...
		const commit = () => {
			const atFloor = vLo <= slLo + 1e-9,
				atCeil = vHi >= slHi - 1e-9;
				...
				try {
					...
					if (keep.length) {
						...
						for (let k = 0; k < wPos.count; k += 2) {
							p0.fromBufferAttribute(wPos, k);
							p1.fromBufferAttribute(wPos, k + 1);
							paretoGroup.add(mkTube(p0, p1, 0.35, 0x98c379, 0.85));
						}
					}
					for (const [, i] of frontA) onFrontier[i] = 1;
				} catch {
					/* degenerate hull (near-coplanar points): skip the mesh, no line either */
				}
			}
	...
	function axText(mode) {
		if (mode === 'space') {
			return (
				`<span title="${tr('lmspace.axes.priceTip')}"><b style="color:#e5c07b">X</b> ${tr('lmspace.axes.price')}</span><br>` +
		...
		return (
			`<span title="${tr('lmspace.axes.releasedTip')}"><b style="color:#e5c07b">X&#8226;Z</b> ${tr('lmspace.axes.released')}</span><br>` +
	...
	function renderRadiusField() {
		...
		if (!el) return;
		el.innerHTML =
			'<span class="cyc" style="--g:#c678dd">r&nbsp;' +
   ...
   sitting in a row of its own implying it is a third mode. */
	...
	paintProjCycle = wireCycle(
		...
		() => projMode,
		(v) => setProjection(v),
		(k) => tr(k === 'persp' ? 'lmspace.hud.persp' : 'lmspace.hud.ortho')
	);
	syncProjUI();
	...
	if (NARROW()) {
		setCollapsed($('modes'), true);
		setCollapsed(legend, true);
	}
	...
	let raceT = 0,
		racePick = -1,
		raceDone = false,
		racePaused = false; // raceOn itself is declared above, before the view cycle
	...
	$('race-restart').onclick = () => {
		racePaused = true;
		raceSeek(0);
	};
	$('race-back').onclick = () => {
		racePaused = true;
		raceSeek(raceT - RACE_STEP);
	};
	$('race-fwd').onclick = () => {
		racePaused = true;
		raceSeek(raceT + RACE_STEP);
	...
	function buildClusterViz(cl) {
		...
		cl.forEach((members, k) => {
			...
			if (clusterMode === 'hull' || clusterMode === 'both') {
				...
				if (members.length >= 4) {
					try {
						...
						/* degenerate hull: the links still show the grouping */
					}
				}
			}
			...
			if (clusterMode === 'link' || clusterMode === 'both') {
				const pts = [];
				for (const [a, b] of spanningLinks(members)) {
					pts.push(gPos[a], gPos[b]);
				}
				if (pts.length) {
					clusterGroup.add(
						new THREE.LineSegments(
	...
	const CLUSTER_MODE_LABEL = {
		hull: 'lmspace.hud.hull',
		link: 'lmspace.hud.link',
		both: 'lmspace.hud.both',
		off: 'common.off'
	};
	$('g-hull').onclick = () => {
	...
	const gPosX = new Float64Array(N),
		gPosY = new Float64Array(N),
		gPosZ = new Float64Array(N);
	const cenX = new Float64Array(64),
		cenY = new Float64Array(64);
	const cenZ = new Float64Array(64),
		cenN = new Float64Array(64);
	...
	function showTip(target) {
     ...
     and using viewport coordinates directly offsets every tip by that inset. */
		...
		tipEl.style.setProperty(
			'--arrow',
			Math.max(10, Math.min(r.left + r.width / 2 - left, tw - 10)) + 'px'
		);
	}
   ...
   stopped the view's own GUIDE button, itself a titled button, from opening. */
	onDoc('click', (e) => {
		...
		if (
			!inStage ||
			(t.tagName === 'BUTTON' &&
				!t.classList.contains('hint-tip') &&
				!t.classList.contains('has-tip'))
		) {
	...
	$('meta').innerHTML =
		...
		`v${DATA.v} &middot; ` +
		(DATA.fetchedAt
			? tr('lmspace.meta.fetched', {
					when: new Date(DATA.fetchedAt).toLocaleString(get(locale), {
						dateStyle: 'medium',
						timeStyle: 'short'
					})
				})
   ...
   ways, so a real frame can never be mistaken for a scripted one. */
	...
	function runFrame(dt) {
		...
		const q = new THREE.Quaternion().setFromEuler(
			new THREE.Euler(yawPitch.pitch, yawPitch.yaw, 0, 'YXZ')
		);
     ...
     press covers the same distance whatever the frame rate. */
		...
		if (gravityOn) {
			...
			if (performance.now() - gravPanelAt > 600) {
				gravPanelAt = performance.now();
				renderGravityPanel();
			}
		}
		...
		if (paretoOn) {
			if (morph >= 1 && !paretoGroup.visible) {
				paretoGroup.visible = true;
				paretoBGroup.visible = true;
				buildParetoViz();
				paretoStaleAt = performance.now();
