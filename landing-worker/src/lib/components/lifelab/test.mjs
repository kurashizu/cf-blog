...
function classify(name, gens = 900) {
	...
	for (let gen = 0; gen <= gens; gen++) {
		...
		if (prev) {
			...
			if (!dx && !dy)
				return period === 1
					? { kind: 'still', pop: live.length }
					: { kind: 'oscillator', period, pop: live.length };
...
check(
	'it can pan and draw',
	['pan', 'draw'].every((t) => LEVELS[0].tools.includes(t))
);
...
// ── every pattern is distinct ────────────────────────────────────────────
{
	...
	for (const n of Object.keys(RLES)) {
		...
		if (shapes.has(key)) {
			dupes++;
			console.log('       ' + n + ' is identical to ' + shapes.get(key));
		}
		shapes.set(key, n);
	}
	check(
		'every pattern is a different shape',
		dupes === 0,
		shapes.size + ' shapes for ' + Object.keys(RLES).length + ' names'
	);
}
...
const EXPECT = {
	...
	blinker: { kind: 'oscillator', period: 2 },
	toad: { kind: 'oscillator', period: 2 },
	beacon: { kind: 'oscillator', period: 2 },
	clock: { kind: 'oscillator', period: 2 },
	pulsar: { kind: 'oscillator', period: 3 },
	figure8: { kind: 'oscillator', period: 8 },
	pentadec: { kind: 'oscillator', period: 15 },

	glider: { kind: 'ship', period: 4, diagonal: true },
	lwss: { kind: 'ship', period: 4 },
	mwss: { kind: 'ship', period: 4 },
	hwss: { kind: 'ship', period: 4 },
	flotilla: { kind: 'ship', period: 4 },
	loafer: { kind: 'ship', period: 7 }, // c/7, as the label claims
	copperhead: { kind: 'ship', period: 10 }, // c/10

	rpent: { kind: 'unbounded' },
	acorn: { kind: 'unbounded' },
	bunnies: { kind: 'unbounded' },
	rabbits: { kind: 'unbounded' },
	switchEngine: { kind: 'unbounded' },
	gosperGun: { kind: 'unbounded' },
	diehard: { kind: 'dies' },
	// From the Golly set: the small ones are cheap enough to run here.
	blom: { kind: 'unbounded' },
	lidka: { kind: 'unbounded' },
	iwona: { kind: 'unbounded' },
	justyna: { kind: 'unbounded' },
	pufferTrain: { kind: 'unbounded' },
	quad20: { kind: 'unbounded' },
	spacefiller: { kind: 'unbounded' }
};
...
for (const [name, want] of Object.entries(EXPECT)) {
	...
	const desc =
		got.kind === 'ship'
			? `ship p${got.period} (${got.dx},${got.dy})`
			: got.kind === 'oscillator'
				? `oscillator p${got.period}`
				: got.kind === 'dies'
					? `dies at gen ${got.gen}`
					: got.kind;
	check(
		`${patternMeta(name).label} is ${want.kind}${want.period ? ' period ' + want.period : ''}`,
		ok,
		'-> ' + desc
	);
}
...
for (const n of ['rpent', 'acorn', 'bunnies', 'rabbits']) {
	const r = classify(n);
	check(
		`${RLES[n].label} runs a long way from a small start`,
...
}
{
	...
	check(
		'GLIDER SINK eats the glider and repairs itself',
		r.final === 7,
		r.final + ' cells left (the eater is 7)'
	);
}
{
	...
	check(
		'GUN + SINK stays bounded while the gun fires for ever',
		hi < 200 && lo > 0,
		`population settles between ${lo} and ${hi}`
	);
}
...
// invisible until something is placed and looks wrong.
{
	...
	const bad = keys.filter((k) => {
		try {
			return !patternMeta(k) || !pattern(k).cells.length;
		} catch {
			return true;
		}
	});
	...
	const sized = Object.entries(EXT).filter(([k, e]) => {
		const p = pattern(k);
		return p.w !== e.w || p.h !== e.h;
	});
	check(
		'every Golly entry decodes to its declared size',
		sized.length === 0,
		sized.map(([k]) => k).join(', ')
	);
	check(
		'the Turing machine is there and is the size Rendell built',
		EXT.turing && pattern('turing').w === 1714 && pattern('turing').h === 1647,
		EXT.turing
			? pattern('turing').w +
					'x' +
					pattern('turing').h +
					', ' +
					pattern('turing').cells.length +
					' cells'
			: 'missing'
	);
}
...
// lost in either direction would make a custom pattern silently wrong.
{
	...
	for (const k of [...Object.keys(RLES), 'stargate', 'p52gun']) {
		...
		if (a !== b) {
			bad++;
			console.log('       ' + k + ' changed on the way through RLE');
		}
	}
	check('encode→decode returns every cell', bad === 0);
	const parsed = parseRLE(
		'#N Glider\n#C the smallest ship\nx = 3, y = 3, rule = B3/S23\nbob$2bo$\n3o!\n'
	);
	check(
		'parseRLE reads a whole file',
		parsed.name === 'Glider' && parsed.w === 3 && parsed.rle === 'bob$2bo$3o!',
		JSON.stringify(parsed)
	);
	const g = pattern('glider');
	const r1 = transformCells(g, 1, false),
		f1 = transformCells(g, 0, true);
	check(
		'rotate keeps the cell count and swaps the box',
		r1.cells.length === 5 && r1.w === g.h && r1.h === g.w
	);
	check(
		'flip mirrors x',
		f1.cells.every(([x]) => x >= 0 && x < g.w) &&
			f1.cells.some(([x, y]) => !g.cells.some(([gx, gy]) => gx === x && gy === y))
	);
	...
	for (const key of ['glider', 'lwss', 'rpent', 'acorn']) {
		...
		for (const op of [
			...
			if (!sameCells(transformCells(p, st.rot, st.flip), shown)) {
				ok = false;
				console.log('       ' + key + ' diverges after ' + op);
				break;
			}
		}
	}
