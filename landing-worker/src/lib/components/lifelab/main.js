...
function bind() {
	...
	pieceBar = document.createElement('div');
	pieceBar.id = 'piecebar';
	pieceBar.className = 'fbar';
	pieceBar.hidden = true;
...
const ICONS = {
	...
	reset:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M6 2a4 4 0 1 1-3.87 5H0l2.7-3.2L5.4 7H4.2A2 2 0 1 0 6 4z" fill="currentColor"/></svg>',
	...
	erase:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M1 1h10v10H1zm2 2v6h6V3z" fill="currentColor"/></svg>',
	...
	clear:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M2 1L6 5l4-4 1 1-4 4 4 4-1 1-4-4-4 4-1-1 4-4L1 2z" fill="currentColor"/></svg>',
	...
	trace:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M0 9h2v2H0zM3 6h2v2H3zM6 3h2v2H6zM9 0h3v3H9z" fill="currentColor"/></svg>',
	ghost:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M1 1h5v5H1zm1 1v3h3V2z" fill="currentColor"/><path d="M6 6h5v5H6z" fill="currentColor" opacity=".45"/></svg>',
	...
	close:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M6 8.5L1.5 4l1-1L6 6.5 9.5 3l1 1z" fill="currentColor"/></svg>',
	...
	check:
		'<svg width="11" height="11" viewBox="0 0 12 12"><path d="M0 6h2v2H0zM2 8h2v2H2zM4 6h2v2H4zM6 4h2v2H6zM8 2h2v2H8zM10 0h2v2h-2z" fill="currentColor"/></svg>',
...
function buildTopbar() {
	...
	const addTool = (name, icon, label) => {
		el.toolBtns[name] = mkBtn(icon, label, () => {
			S.tool = name;
			syncTools();
		});
	};
...
function thumbOf(p, color, size = 26) {
	...
	const W = Math.max(8, Math.round(p.w * sc)),
		H = Math.max(8, Math.round(p.h * sc));
...
function loadFolded() {
	...
	try {
		trayFolded = JSON.parse(localStorage.getItem('lifelab.tray.folded') || '{}') || {};
	} catch {
		trayFolded = {};
	}
	return trayFolded;
...
function saveFolded() {
	try {
		localStorage.setItem('lifelab.tray.folded', JSON.stringify(trayFolded || {}));
	} catch {}
}
...
function buildTray() {
	...
	search.id = 'trsearch';
	search.type = 'search';
	search.placeholder = tr('lifelab.ui.searchPlaceholder');
	search.value = trayQuery;
	search.autocomplete = 'off';
	search.spellcheck = false;
	search.oninput = () => {
		trayQuery = search.value.trim().toLowerCase();
		renderTray();
	};
	tray.appendChild(search);
	traySecs = document.createElement('div');
	traySecs.id = 'trsecs';
...
function renderTray() {
	...
	sections.push({
		id: 'custom',
		label: tr('lifelab.ui.customLabel'),
		hint: tr('lifelab.ui.customHint'),
		items: custom.list().map((c) => c.key),
		mine: true
	});
	let colour = 0;
	for (const sec of sections) {
		const items = sec.items.filter((k) => {
			...
			return (m.label + ' ' + (m.note || '') + ' ' + (m.credit || '') + ' ' + sec.label)
				.toLowerCase()
				.includes(q);
		});
		...
		for (const k of items)
			grid.appendChild(stampButton(k, PCOLORS[colour++ % PCOLORS.length], sec.mine));
		if (sec.mine) {
			const add = document.createElement('button');
			add.className = 'stamp tradd';
			add.innerHTML = ICONS.sel + '<span>' + tr('lifelab.ui.fromSelection') + '</span>';
			add.title = tr('lifelab.ui.fromSelectionHint');
			add.onclick = () => {
				S.tool = 'sel';
				syncTools();
				tlog('> ' + tr('lifelab.log.selectHint'), 't-sys');
			};
			const paste = document.createElement('button');
			paste.className = 'stamp tradd';
...
function stampButton(k, color, mine) {
	...
	b.innerHTML =
		...
		'</span>' +
		(p.w * p.h >= 2500
			? '<i' + (big ? ' class="big"' : '') + '>' + p.w + '&times;' + p.h + '</i>'
			: '');
	b.title =
		tr('lifelab.ui.stampTooltip', { label: m.label, w: p.w, h: p.h, cells: p.cells.length }) +
		(m.note ? '\n' + m.note : '') +
		(m.credit ? '\n' + m.credit : '') +
	...
	if (mine) {
		...
		x.className = 'del';
		x.innerHTML = '&#10005;';
		x.title = tr('lifelab.ui.deletePattern');
		x.onclick = (e) => {
			e.stopPropagation();
			custom.remove(k);
			renderTray();
			tlog('> ' + tr('lifelab.log.deletedPattern', { label: m.label }), 't-warn');
		};
		b.appendChild(x);
	}
	el.stampBtns[k] = b;
...
function pieceCenterAt(pc, cx, cy) {
	pc.x = cx - (pc.w >> 1);
	pc.y = cy - (pc.h >> 1);
}
function pieceHit(c) {
	const pc = S.piece;
	return !!(pc && c && c.x >= pc.x && c.y >= pc.y && c.x < pc.x + pc.w && c.y < pc.y + pc.h);
}
...
function pickPiece(k) {
	...
	if (p.w > S.L.w || p.h > S.L.h) {
		...
		showMsg(
			...
			tr('lifelab.dialog.biggerThanDishBody', {
				label: m.label,
				pw: p.w,
				ph: p.h,
				dw: S.L.w,
			...
			[
				{
					label: tr('lifelab.dialog.enlargeAndPlace'),
					fn: () => {
						hideMsg();
						commitPiece();
						resizeDish(nw, nh);
	...
	const pc = makePiece(
		...
		p.w,
		p.h,
		{ key: k, label: m.label, follow: !isTouch() }
	);
...
function liftSelection() {
	...
	if (!raw.length) {
		deny(tr('lifelab.log.nothingInBox'));
		return;
	}
	pushUndo();
	let bx = Infinity,
	...
	S.piece = makePiece(n.cells, n.w, n.h, {
		x: bx,
		y: by,
		label: tr('lifelab.ui.selection'),
		origin: { x: bx, y: by, cells: n.cells.slice() }
	});
	syncPieceBar(true);
...
function commitPiece(keep) {
	...
	for (const [x, y] of pc.cells) {
		const gx = pc.x + x,
			gy = pc.y + y;
		if (gx < 0 || gy < 0 || gx >= S.L.w || gy >= S.L.h) {
			lost++;
			continue;
		}
		S.eng.set(gx, gy, 1);
	...
	tlog(
		'> ' +
			(lost
				...
				: tr('lifelab.log.placed', { label: pc.label, x: pc.x, y: pc.y })),
		lost ? 't-warn' : 't-sys'
	);
...
function cancelPiece() {
	...
	if (!pc) return;
	if (pc.origin)
		for (const [x, y] of pc.origin.cells) S.eng.set(pc.origin.x + x, pc.origin.y + y, 1);
	S.piece = null;
	syncPieceBar(true);
}
function deletePiece() {
	...
	tlog(
		'> ' +
			(pc.origin
				? tr('lifelab.log.deletedCells', { n: pc.cells.length })
				: tr('lifelab.log.dropped', { label: pc.label })),
		't-sys'
	);
	S.piece = null;
	syncPieceBar(true);
...
function rotatePiece() {
	...
	pc.w = t.w;
	pc.h = t.h;
	pc.x = Math.round(cx - pc.w / 2);
	pc.y = Math.round(cy - pc.h / 2);
...
function savePiece() {
	...
	if (!pc) return;
	showPrompt(
		tr('lifelab.dialog.saveAsPatternTitle'),
		tr('lifelab.dialog.saveAsPatternBody', { w: pc.w, h: pc.h, cells: pc.cells.length }),
		pc.key ? pc.label : tr('lifelab.ui.myPattern'),
		(name) => {
...
function copyPiece() {
	...
	if (navigator.clipboard?.writeText)
		navigator.clipboard
			...
			.then(done, () =>
				showPrompt(
					tr('lifelab.dialog.rleTitle'),
					tr('lifelab.dialog.rleClipboardRefused'),
					'',
					null,
					text
				)
		...
		showPrompt(tr('lifelab.dialog.rleTitle'), tr('lifelab.dialog.rleCopyFromHere'), '', null, text);
}
...
function pasteClip() {
	...
	if (!navigator.clipboard?.readText) {
		deny(tr('lifelab.log.nothingToPaste'));
		return;
	}
	navigator.clipboard.readText().then(
		(t) => {
			...
			if (!parsed.rle) {
				deny(tr('lifelab.log.clipboardNoRle'));
				return;
			}
...
function syncPieceBar(rebuild) {
	...
	if (rebuild || pieceBar.hidden) {
		const color =
			(pc.key && el.stampBtns[pc.key] && el.stampBtns[pc.key].style.color) ||
			(pc.origin ? '#e5c07b' : '#56b6c2');
		const touch = isTouch();
		pieceBar.hidden = false;
		pieceBar.innerHTML =
			'<span class="sbthumb" style="color:' +
			color +
			'" title="' +
			tr('lifelab.ui.asLanded') +
			'">' +
			thumbOf(pc, color, 30) +
			'</span>' +
			'<b style="color:' +
			...
			pc.cells.length +
			'</small>' +
			'<button class="tb" id="pb-rot" title="' +
			tr('lifelab.ui.rotateHint') +
			'">' +
			ICONS.rot +
			'<span>' +
			tr('lifelab.ui.rotate') +
			'</span></button>' +
			'<button class="tb" id="pb-flip" title="' +
			tr('lifelab.ui.flipHint') +
			'">' +
			ICONS.flip +
			'<span>' +
			tr('lifelab.ui.flip') +
			'</span></button>' +
			'<button class="tb" id="pb-copy" title="' +
			tr('lifelab.ui.copyHint') +
			'">' +
			ICONS.draw +
			'<span>' +
			tr('lifelab.ui.copy') +
			'</span></button>' +
			'<button class="tb" id="pb-save" title="' +
			tr('lifelab.ui.saveHint') +
			'">' +
			ICONS.lab +
			'<span>' +
			tr('lifelab.ui.save') +
			'</span></button>' +
			'<button class="tb go" id="pb-drop" title="' +
			tr('lifelab.ui.dropHint') +
			'">' +
			ICONS.check +
			'<span>' +
			tr('lifelab.ui.drop') +
			'</span></button>' +
			'<button class="tb" id="pb-again" title="' +
			tr('lifelab.ui.stampHint') +
			'">' +
			ICONS.soup +
			'<span>' +
			tr('lifelab.ui.stamp') +
			'</span></button>' +
			'<button class="tb bad" id="pb-del" title="' +
			(pc.origin ? tr('lifelab.ui.deleteCellsHint') : tr('lifelab.ui.throwAwayHint')) +
			'">' +
			ICONS.wipe +
			'<span>' +
			tr('lifelab.ui.delete') +
			'</span></button>' +
			'<em>' +
			(pc.follow
				? tr('lifelab.ui.followHint')
				: touch
					? tr('lifelab.ui.touchHint')
					: pc.origin
						? tr('lifelab.ui.dragHintOrigin')
						: tr('lifelab.ui.dragHintCancel')) +
			'</em>';
...
function placePieceBar() {
	const pc = S.piece;
	if (!pc || !pieceBar || pieceBar.hidden) return;
	const wrap = cv.getBoundingClientRect();
	const bw = pieceBar.offsetWidth,
		bh = pieceBar.offsetHeight;
	let left, top;
	if (pc.follow) {
		left = (wrap.width - bw) / 2;
		top = 8;
	} else {
...
function undoEdit() {
	...
	if (!snap) {
		deny(tr('lifelab.log.nothingToUndo'));
		return;
	}
	undoBytes -= snap.a.byteLength;
	if (S.piece) {
...
function selRect() {
	const q = S.sel;
	if (!q) return null;
	const x = Math.max(0, Math.min(q.x0, q.x1)),
		y = Math.max(0, Math.min(q.y0, q.y1));
	const x2 = Math.min(S.L.w - 1, Math.max(q.x0, q.x1)),
		y2 = Math.min(S.L.h - 1, Math.max(q.y0, q.y1));
...
function importRLE() {
	showPrompt(
		tr('lifelab.dialog.pasteRleTitle'),
		tr('lifelab.dialog.pasteRleBody'),
		tr('lifelab.dialog.namePlaceholder'),
		(name, text) => {
			const parsed = parseRLE(text || '');
			let d;
			try {
				d = parsed.rle ? normalizeCells(decodeRLE(parsed.rle).cells) : null;
			} catch {
				d = null;
			}
			if (!d || !d.cells.length) {
				deny(tr('lifelab.log.notRle'));
				return;
			}
			const key = custom.add(
				name || parsed.name || tr('lifelab.ui.pasted'),
				d.cells,
				d.w,
				d.h,
				parsed.comments[0]
			...
			tlog(
				'> ' +
					tr('lifelab.log.imported', {
						label: patternMeta(key).label,
						w: d.w,
						h: d.h,
						n: d.cells.length
					}),
...
function cycleDish() {
	...
	const lost =
		w < S.L.w || h < S.L.h
			? S.eng.pop - S.eng.rectCount({ x: 0, y: 0, w: Math.min(w, S.L.w), h: Math.min(h, S.L.h) })
			: 0;
	if (lost > 0) {
		showMsg(
			tr('lifelab.dialog.shrinkTitle'),
			tr('lifelab.dialog.shrinkBody', { lost, w, h }),
			[
				{
					label: tr('lifelab.dialog.shrinkIt'),
					fn: () => {
						hideMsg();
						resizeDish(w, h);
					}
...
function rewindCap() {
	return Math.max(3, Math.min(40, Math.floor(1.6e7 / (S.L.w * S.L.h))));
}
...
function wipeSave() {
	if (S.eng.pop === 0) {
		deny(tr('lifelab.log.dishAlreadyEmpty'));
		return;
	}
	showMsg(
...
function soup() {
	...
	for (let y = 0; y < S.L.h; y++)
		for (let x = 0; x < S.L.w; x++) e.set(x, y, Math.random() < 0.12 ? 1 : 0);
...
function stepBack() {
	...
	if (!prev) {
		deny(tr('lifelab.log.rewindNothing'));
		return;
	}
	S.running = false;
	S.eng.restore(prev);
...
function syncRun() {
	if (!el.run) return;
	el.run.innerHTML =
		(S.running ? ICONS.pause : ICONS.play) +
		'<span>' +
		(S.running ? tr('lifelab.ui.pause') : tr('lifelab.ui.run')) +
		'</span>';
}
function syncTools() {
	...
	for (const [n, b] of Object.entries(el.stampBtns)) {
		const on = held === n;
		b.classList.toggle('on', on);
		b.style.borderColor = on ? b.style.color : '';
	}
}

/* ---------------- editing ---------------- */
function inGrid(c) {
	return c.x >= 0 && c.y >= 0 && c.x < S.L.w && c.y < S.L.h;
}

function canEditCell(x, y, quiet) {
	if (!S.L.sandbox && S.phase !== 'edit') {
		if (!quiet) deny(tr('lifelab.log.cannotEditWhileRunning'));
		return false;
	}
...
function paint(x, y, v) {
	...
	if (
		...
		S.L.budget &&
		!S.eng.get(x, y) &&
		S.eng.pop - S.presetPop >= S.L.budget
	) {
		deny(tr('lifelab.log.budgetExhausted', { n: S.L.budget }));
		return;
	}
	S.eng.set(x, y, v);
}
...
function showMsg(title, text, btns, bad) {
	...
	btns.forEach((b) => {
		...
		bt.className = 'tb';
		bt.innerHTML = '<span>' + b.label + '</span>';
		bt.onclick = b.fn;
		row.appendChild(bt);
	});
	box.append(h, p, row);
...
function showPrompt(title, text, placeholder, onOk, body = '', withBody = false) {
	...
	if (onOk) {
		...
		ok.className = 'tb';
		ok.innerHTML = '<span>' + tr('lifelab.dialog.ok') + '</span>';
	...
	cancel.className = 'tb';
	cancel.innerHTML =
		'<span>' + (onOk ? tr('lifelab.dialog.cancel') : tr('lifelab.dialog.close')) + '</span>';
	...
	if (input)
		input.onkeydown = (e) => {
			if (e.key === 'Enter') onOk(input.value.trim(), area ? area.value : '');
		};
}
...
function toCell(e) {
	...
	const mx = e.clientX - r.left,
		my = e.clientY - r.top;
	return {
		x: Math.floor((mx - S.cam.x) / S.cam.s),
		y: Math.floor((my - S.cam.y) / S.cam.s),
		mx,
		my
	};
}
...
function bindInput() {
	...
	onPointerMove = (e) => {
		...
		if (S.drag.mode === 'piece') {
			const c = toCell(e);
			pc.x = S.drag.px + (c.x - S.drag.cx);
			pc.y = S.drag.py + (c.y - S.drag.cy);
			return;
		}
		if (S.drag.mode === 'select') {
			const c = toCell(e);
			S.sel.x1 = Math.max(0, Math.min(S.L.w - 1, c.x));
			S.sel.y1 = Math.max(0, Math.min(S.L.h - 1, c.y));
			return;
		}
		if (S.drag.mode === 'pan') {
			...
			const c = toCell(e);
			if (inGrid(c)) paint(c.x, c.y, S.paintVal);
		} else if (S.drag.mode === 'erase') {
			const c = toCell(e);
			if (inGrid(c)) paint(c.x, c.y, 0);
		}
	};
	onPointerUp = (e) => {
	...
	onKeyDown = (e) => {
		...
		if (mod && !e.altKey) {
			if (e.code === 'KeyZ' && !e.shiftKey) {
				e.preventDefault();
				undoEdit();
			} else if (e.code === 'KeyC' && S.piece) {
				e.preventDefault();
				copyPiece();
			} else if (e.code === 'KeyV') {
				e.preventDefault();
				pasteClip();
			} else if (e.code === 'KeyD' && S.piece) {
				e.preventDefault();
				commitPiece(true);
			} else if (e.code === 'KeyS' && S.piece) {
				e.preventDefault();
				savePiece();
			}
			return;
		...
		if (e.code === 'Space') {
			...
			nudgePiece(
				e.code === 'ArrowLeft' ? -n : e.code === 'ArrowRight' ? n : 0,
				e.code === 'ArrowUp' ? -n : e.code === 'ArrowDown' ? n : 0
			);
...
function updateImage(v) {
	const L = S.L,
		...
		st = e.stride,
		a = e.a;
	const [pr, pg, pb] = THEME.panel,
		[ar, ag, ab] = THEME.accent;
	for (let y = v.y0; y < v.y1; y++) {
		let si = (y + 1) * st + v.x0 + 1,
			gi = y * L.w + v.x0,
			di = gi * 4;
...
function drawGrid() {
	...
	ctx.strokeStyle = rgba(THEME.fg, 0.06);
	ctx.lineWidth = 1;
	ctx.beginPath();
	for (let x = x0; x <= x1; x++) {
		const sx = S.cam.x + x * S.cam.s;
		ctx.moveTo(sx, S.cam.y + y0 * S.cam.s);
		ctx.lineTo(sx, S.cam.y + y1 * S.cam.s);
	}
	for (let y = y0; y <= y1; y++) {
		const sy = S.cam.y + y * S.cam.s;
		ctx.moveTo(S.cam.x + x0 * S.cam.s, sy);
		ctx.lineTo(S.cam.x + x1 * S.cam.s, sy);
	}
	ctx.stroke();
}
...
function drawPiece() {
	...
	ctx.setLineDash([4, 3]);
	ctx.strokeStyle = pc.origin ? 'rgba(229,192,123,.9)' : 'rgba(86,182,194,.8)';
	ctx.lineWidth = 1;
	ctx.strokeRect(
		Math.round(bx) + 0.5,
		Math.round(by) + 0.5,
	...
	const ok = 'rgba(86,182,194,.6)',
		bad = 'rgba(224,108,117,.6)';
	for (const [x, y] of pc.cells) {
		const gx = pc.x + x,
			gy = pc.y + y;
...
function drawTrace() {
	...
	cx = cx / cells.length + 0.5;
	cy = cy / cells.length + 0.5;
...
function drawGhostFrame() {
	...
	for (let y = v.y0 - 1; y < v.y1 + 1; y++)
		for (let x = v.x0 - 1; x < v.x1 + 1; x++)
			if (x >= 0 && y >= 0 && x < S.L.w && y < S.L.h && S.eng.get(x, y))
				sim.set(x - v.x0 + 1, y - v.y0 + 1, 1);
	...
	for (let y = v.y0; y < v.y1; y++)
		for (let x = v.x0; x < v.x1; x++) {
			...
			for (let dy = -1; dy <= 1; dy++)
				for (let dx = -1; dx <= 1; dx++) if ((dx || dy) && S.eng.get(x + dx, y + dy)) n++;
			if (!n) continue;
			const [sx, sy, sw, sh] = cellRect(x, y, 1, 1);
			// Three is the one that matters: that cell is about to be born.
			ctx.fillStyle = n === 3 ? 'rgba(152,195,121,.9)' : rgba(THEME.fg, 0.35);
...
function drawHover() {
	...
	ctx.strokeStyle = 'rgba(209,154,102,.5)';
	ctx.lineWidth = 1;
	ctx.strokeRect(sx, sy, sw, sh);
	let n = 0;
	for (let dy = -1; dy <= 1; dy++)
		for (let dx = -1; dx <= 1; dx++) if ((dx || dy) && S.eng.get(c.x + dx, c.y + dy)) n++;
...
function drawDishFrame() {
	...
	ctx.strokeStyle = rgba(THEME.fg, 0.18);
	ctx.lineWidth = 1;
	ctx.strokeRect(
		Math.round(sx) + 0.5,
		Math.round(sy) + 0.5,
...
function drawSelection() {
	...
	ctx.setLineDash([4, 3]);
	ctx.strokeStyle = 'rgba(229,192,123,.9)';
	ctx.lineWidth = 1;
...
function syncPointer(anchor) {
	...
	const gw = guideEl.offsetWidth || 300,
		gh = guideEl.offsetHeight || 70;
...
function frame(t) {
	...
	if (S.piece) {
		placePieceBar();
		cv.style.cursor = S.piece.follow ? 'copy' : pieceHit(S.hover) ? 'move' : 'default';
...
function buildConsoleControl() {
	return {
		...
		step(n = 1) {
			S.running = false;
			for (let i = 0; i < Math.max(1, n | 0); i++) doStep();
			draw();
			updateStats();
			syncRun();
		},
		...
		random(density) {
			...
			const e = S.eng;
			for (let y = 0; y < S.L.h; y++)
				for (let x = 0; x < S.L.w; x++) e.set(x, y, Math.random() < d ? 1 : 0);
			draw();
			updateStats();
		},
		setSpeed(gen) {
			...
			S.speed = i;
			if (el.spd)
				el.spd.querySelector('span').textContent = tr('lifelab.ui.speed', {
					rate: SPEEDS[S.speed]
				});
			return true;
		},
