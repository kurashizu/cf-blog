...
<div
	class="relative z-10 w-full min-h-screen lg:h-screen lg:max-h-screen overflow-x-hidden lg:overflow-hidden font-mono text-sm sm:text-base {themeStyles.text} flex flex-col justify-between select-none p-1.5 sm:p-3 md:p-4 transition-colors duration-200"
>
	<TabBar />

	<div
		class="grid grid-cols-12 lg:grid-cols-[repeat(24,minmax(0,1fr))] gap-1.5 sm:gap-2 flex-1 min-h-0 w-full max-w-full"
	>
...
{#if $consoleOverlayOpen}
	...
	<div
		...
		onclick={() => consoleOverlayOpen.set(false)}
