...
/**
 * The arm64 guest's kernel and root filesystem, for the QEMU machine.
 *
...
export const GET: RequestHandler = async ({
  ...
  const env = platform?.env as
    { ARM_IMAGES?: string; VM_BUCKET?: R2Bucket } | undefined;
  const images = parseImages(env?.ARM_IMAGES);
  const image = images[params.name];
  if (!image) {
    error(404, `No such arm64 image: ${params.name}`);
  }
