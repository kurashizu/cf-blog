...
export function GuestbookMessages({
    ...
    useEffect(() => {
        ...
        return () =>
            window.removeEventListener("guestbook-posted", handlePosted);
    }, []);
    ...
    useEffect(() => {
        fetch("/api/guestbook")
            .then(
                (res) =>
                    res.json() as Promise<{ messages?: GuestbookMessage[] }>,
            )
            .then((data) => {
            ...
            {showAll &&
                ...
                createPortal(
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        ...
                        />
                        <div
                            className={`relative w-full max-w-2xl max-h-[80vh] bg-bg-card/80 backdrop-blur-xl border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden ${closing ? "animate-slideDown" : "animate-slideUp"}`}
                        >
                            ...
                            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
                                {paginatedMessages.map((msg) => (
                                    <div
                                        key={msg.id}
                                        className="w-full p-4 bg-bg-card/60 backdrop-blur-sm border border-border rounded-xl"
                                    >
