...
The sequencer has **one tempo**, **24 steps per beat**, bar = beats-per-bar × 24 steps (4/4 → 96), **max `MAX_GRID_STEPS` steps** (in `synth.ts`; 32768 as of the Take Five import, was 12288 = 128 bars of 4/4). Note index = `108 − MIDI note` (C4 = 48; only 0..87 fit, drop the rest). Up to **8 notes per cell**, **8 tracks**.

- Constant tempo, song fits: steps per MIDI beat = 24, bpm = the file's. Prefer this whenever it fits — full resolution is the only way to reproduce syncopation exactly (see below).
- Too long at 24 steps/beat: first check whether raising `MAX_GRID_STEPS` (and the three other spots that mirror it — grep for it) is cheap enough to just fit the song at full resolution instead of degrading it. It's a real tradeoff though, not a free one: every track allocates a JS array of this length for its grid and its accents (8 tracks × steps, one array object per step), so a much bigger cap means a much bigger one-time allocation on every new project or reset — this is a memory/GC cost, and exporting patches gzip-compressed does *not* offset it, since compression only shrinks the exported file, not the in-memory grids. Bump it in modest steps (12288 → 16384 → 32768, not straight to six figures) and only as far as the song actually needs. Only fall back to trimming or half time if the song is enormous (Spain-scale or longer) and a sane cap raise still doesn't fit it.
- **Half time (12 steps per MIDI beat, bpm ÷ 2, meter unchanged) loses precision, not just resolution.** It works cleanly for tempo (same real-time speed, Spain: 230→115 BPM) and for any meter's bar math (5/4 at 180 → sequencer 5/4 at 90 is still whole steps per bar). But any note whose tick isn't a multiple of `PPQ/12` rounds to the nearest half-time step, and a MIDI file quantized at 1/24-beat (most are) puts exactly this kind of note on its syncopated hits — a swung ride, an off-the-beat comp stab. Take Five's ride cymbal (hits like tick 540 = step 67.5 at 12/beat, step 135.0 at 24/beat) rounded to the wrong step at half time and audibly broke the tune's signature 3+2 feel; the fix was full resolution plus a higher `MAX_GRID_STEPS`, not a smarter rounding rule. Before choosing half time, check whether the source's own tempo-relative ticks (`tick / PPQ * 24`) are already integers for the syncopated/characteristic hits — if yes, half time will mangle exactly those.
...
**Ducking** (`duckSource` = drum track id, `duckKeys` = note indexes, `duckDepth` 0–1): bass ducks on the kick (0.45), chords duck on the snare (0.2), a lead over a *sparse* kit can duck on every hit lightly (0.25, Spain). Tracks that should not duck: `duckSource: -1, duckKeys: [], duckDepth: 0`. **Check the drum part's density before ducking the lead on `duckKeys: []` (every hit).** It reads fine against a sparse kit, but if the kit's busiest voice (typically a ride or hi-hat ostinato) fires on most steps, the dip+hold+release never fully releases between hits and the "duck on everything" turns into a near-constant volume cut — this quietly ruined Take Five's sax against its continuous ride pattern. Either duck on specific sparse `duckKeys` (e.g. just the kick or snare) or don't duck the melody at all.
