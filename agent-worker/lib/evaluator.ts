...
class Lexer {
    ...
    tokenize(): Token[] {
        ...
        while (this.pos < this.input.length) {
            ...
            if (
                /\d/.test(ch) ||
                (ch === "." && /\d/.test(this.input[this.pos + 1] || ""))
            ) {
    ...
    private readOperator(): Token {
        // Check 3-character operators first so `===` isn't misread as `==` + `=`.
    ...
    try {
        for (const pat of FORBIDDEN_PATTERNS) {
            if (pat.test(code)) {
                ...
                return {
                    success: false,
                    error: `${match?.[0] ?? pat.source} is not allowed`,
                };
            }
        }
        if (/^\s*\{/.test(code) && /\}\s*$/.test(code.trim())) {
            return {
                success: false,
                error: "Object/block literals not allowed",
            };
        }
        if (/;\s*$/.test(code.trim())) {
