...
export default function AdminGuestbook() {
  ...
  const handleApprove = async (id: string) => {
    setActionLoading(id);
    try {
      const res = await fetch(`/admin/api/guestbook/${id}`, { method: 'POST' });
      if (res.ok) {
        setMessages((msgs) => msgs.map((m) => (m.id === id ? { ...m, approved: true } : m)));
      }
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    ...
    setActionLoading(id);
    try {
      const res = await fetch(`/admin/api/guestbook/${id}`, { method: 'DELETE' });
      if (res.ok) {
    ...
    <div>
      ...
      {messages.length === 0 ? (
        ...
        <>
          ...
          <h2 className="section-title mb-3">All Messages ({messages.length})</h2>
          <div className="space-y-3">
            {approved.map((msg) => (
              <Card key={msg.id}>
                <CardContent className="p-4">
